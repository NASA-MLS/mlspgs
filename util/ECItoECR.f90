...
program ECItoECR
  ...
  ! Read the Earth ellipse (default WGS84), the time, and ECI
  ! coordinates, from the command line.  Print ECR coordinates.
...
o:do
    ...
    case ( '-h' )
      ...
      print '(3a)', 'Usage: ', trim(line), ' [options] Time [3 ECI coordinates]'
    ...
    case default
      ...
      do j = 1, 3
        ...
        read ( line, *, iostat=stat, iomsg=iomsg ) eci(j)
        if ( stat /= 0 ) then
  ...
  do
    if ( line /= '' ) then
      line = asciiUTC
      stat = PGS_CSC_ECItoECR (1, line, offsets, ECI, ECR)
      if ( stat /= PGS_S_Success ) then
        print '(a,i0)', 'Status from PGS_CSC_ECItoECR = ', stat
