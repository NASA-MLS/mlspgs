#!/bin/sh
# --------------- live_l2auxcat.sh help
# live_l2auxcat.sh
# a wrapper for l2auxcat
# assuring that the files to l2auxcatted are "pure"
# i.e., not in the middle of being written to
#
# live_l2auxcat.sh [opt1] [opt2] ..  -o output-file file1 [file2] ..
# Special notes
# (1) l2auxcat must be in your PATH
# (2) h5debug must be in your PATH
# (3) split_path.sh must have the same path prefix as live_l2auxcat.sh
# (4) A sub-directory, live_l2auxcat, will be created in your home directory
...
#    -l2auxcat command    Use command instead of l2auxcat
...
# (2) We can create ${HOME}/live_l2auxcat

# usage: see (1) above

# --------------- End live_l2auxcat.sh help
...
echo $@
me="$0"
my_name=live_l2auxcat.sh
I=live_l2auxcat
#l2auxcat=$HOME/bin/l2auxcat
L2AUXCAT=l2auxcat
H5DEBUG=h5debug
...
if [ "$debug" = 1 ]
then
  echo "L2AUXCAT $L2AUXCAT"
  echo "h5debug $H5DEBUG"
  echo "l2auxcat_opts $l2auxcat_opts"
  echo "list $list"
fi
...
if [ "$dryrun" = "yes" ]
then
  echo $L2AUXCAT $l2auxcat_opts -o $outfile $live
else
  $L2AUXCAT $l2auxcat_opts -o $outfile $live
  returnstatus="$?"
fi
if [ "$cleanup" = 1 ]
then
