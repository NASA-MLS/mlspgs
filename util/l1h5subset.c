 ...
 * This version of it has been hacked to allow the datasets themselves to 
 * be sub-setted by the first index. This is supposed to be of use for 
 * MLS Level 1 RAD and OA files which have huge sizes and for which the 
 * first index of all datasets is the MAF index.

 * Compile it like this:
   cc -g -o l1h5subset l1h5subset.c -lhdf5 
 * and run it like this:
   ./l1h5subset -a nattr -d  varfoo,varbar,varbaz -g groups <infile> <outfile>
...
int main(int argc , char *argv[] ){
  ...
  if (argc < 3){
    printf("Usage:\n");
    printf("l1h5subset -a nattr -d dsets -g groups -m firstmaf:lastmaf <infile> <outfile>\n");
    printf("where dsets is a comma separated list of datasets to copy\n"); 
    printf("groups is a comma-sep. list of groups to copy the whole of \n");
    printf("use -g \"/\" to copy all datasets in the file\n");
    printf("nattr is the number of attributes to copy for each dataset\n");
    printf(" -v : verbose \n");
    exit(3);
  }
  
  /* Initialise various variables so that sensible defaults are used if 
     options are not set by the user */
  invarstring=NULL;
  ingroupstring=NULL;
  optval=0;
  verbose=0;
  ...
  first=-1;
  last=-1;
  invarstring=(char *) malloc(8);
  varstring=(char *) malloc(8);
  strcpy(varstring,"dummy");
  strcpy(invarstring,"dummy");
  ...
  while(optval >= 0){
    ...
    if(optvalc=='v'){
      ...
      if(verbose) printf("arg to -d was%s\n",optarg);
    }else if(optvalc=='g'){
	...
	printf("Doing %d attrs for each dataset \n",global_nattrs);
      ...
      sscanf(optarg,"%d:%d",&first,&last);
      if(verbose)
	printf("Copying MAFs %d through %d \n",first,last);
    }
  }/* End of getopt loop */
  ...
  if(ingroupstring!=NULL){
    ...
    if( strcmp(global_grouplist[0],"/")==0) opdat.getall=1;
    for(i=1;i<n_groups;i++){
      global_grouplist[i]=strtok(NULL,",");
      if( strcmp(global_grouplist[i],"/")==0) opdat.getall=1;
...
herr_t operator(hid_t group_id, const char *member_name, void
  ...
  if(is_wanted==0){
    ...
    if(statbuf.type ==  H5G_GROUP){
      ...
      /* variables local to this block (?is this standard C? It seems 
       * to work.) 
       */
      hid_t old_dspace, new_dspace, ss_dspace, old_dset,mem_dspace;
      hid_t new_dset, type_id ,plist_id, new_plist_id;
      hssize_t offset[10];
      void *databuf;
      int n_elements;
      int *aidx;
      void *aopdata;
      int rank;
      /* Should do size properly */ 
      hsize_t dims[10], maxdims[10],newdims[10],newmaxdims[10],chunkdims[10]; 
      hsize_t start[10],count[10];
      hsize_t *maxdimsp;
      hid_t *new_dsetp;
      ...
      new_plist_id=H5Pcopy(plist_id);  
      rank=H5Sget_simple_extent_ndims(old_dspace);
      if(verbose) printf("Rank is %d\n",rank);
      maxdimsp=maxdims;
      rank=H5Sget_simple_extent_dims(old_dspace, dims,maxdimsp  );
      if(verbose){
	if(maxdimsp == NULL){
	  printf(" maxdims is NULL: dims: ");
	  for(i=0;i<rank;i++){
	    printf("%d ",dims[i]);
	  }
	}
	  ...
	  printf("dims (maxdims): ");
	  for(i=0;i<rank;i++){
	    printf("%d (%d) ",dims[i],maxdims[i]);
	  }
	}
	printf("\n");
      }
      /* Copy old dims to new ones */
      for(i=0;i<rank;i++){
	offset[i]=0;
	chunkdims[i]=1;
	newdims[i]=dims[i];
	newmaxdims[i]=maxdims[i];
      }
      if(first >= 0 && last >= first){
	newdims[0]=last-first+1;
	offset[0]=first;
	if(maxdims[0] >= dims[0]) newmaxdims[0]=newdims[0];
      }
      chunkdims[0]=newdims[0];
      if(verbose){
	printf("newdims (newmaxdims) chunkdims| : ");
	for(i=0;i<rank;i++){
	  printf("%d (%d) %d |  ",newdims[i],newmaxdims[i],chunkdims[i]);
	}
      }
      /* Fix chunking in new property list. */
      H5Pset_chunk(new_plist_id, rank, chunkdims);
      /* Create dataspace for use in new file */
      new_dspace=H5Screate_simple(rank, newdims, newmaxdims ) ;
      /* Create dataspace for required subset of data in old file */
      ss_dspace=H5Scopy(old_dspace);
      error=H5Sselect_hyperslab(ss_dspace,H5S_SELECT_SET, offset, NULL,
				newdims, NULL );
      /* The line below doesnt work */
      /* error=H5Soffset_simple(ss_dspace, offset);*/
	
      /* new_dspace=H5Scopy(old_dspace); not any more */
      type_id=H5Dget_type(old_dset); 
    
      /* This next line makes a new dataset in the new file */

      new_dset=H5Dcreate(newloc_id, member_name, type_id, 
			 new_dspace,  new_plist_id);
	 ...
	 a suitably humongous buffer */
      ...
      error=H5Dread(old_dset,type_id, new_dspace,ss_dspace ,
		    H5P_DEFAULT,databuf ); 
      error=H5Dwrite(new_dset,type_id,new_dspace,new_dspace, 
		     H5P_DEFAULT,databuf ) ;
