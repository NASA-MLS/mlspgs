module FWD_MDL_M
  ...
  use MDBETA, only: NO_T_PHI, MNF => max_no_freq
...
!
    ...
    Real(r8), intent(in) :: Z_GRID(*), T_GRID(*), H_GRID(*)
    Real(r8), intent(in) :: DH_DT_GRID(Nlvl,*), A_GRID(*), N_GRID(*)
    Real(r8), intent(in) :: V_GRID(*)
    integer(i4), intent(in) :: T_INDX
    Real(r8), intent(in) :: CONV_HTS(*)
    Real(r8), intent(in) :: CONV_PRESS(*)
    Real(r8), intent(in) :: CONV_TEMP(*)
    Real(r8), intent(in) :: SPSFUNC(Nlvl,*)
    Real(r8), intent(out) :: CEN_ANGLE
    Real(r8), intent(in) :: EARTH_REF
    Real(r8), intent(in) :: T_TAN(*)
    Real(r8), intent(in) :: ACC(*)
    Real(r8), intent(in) :: THRESHOLD(*)
    type(atmos_comp), intent(in) :: ATMOSPHERIC(*)
    Real(r8), intent(inout) :: CS(Nlvl,no_t_phi,cs_mnf,*)
    Real(r8), intent(out) :: DX_DT(Nlvl,*)
    Real(r8), intent(out) :: D2X_DXDT(Nlvl,*)
    ...
    Real(r8), intent(in) :: G_BASIS(mxco,*)
...
!
    real(r8) :: CSE
    integer(i4) :: CYCLE
    real(r8) :: DEL_OPACITY(N2lvl), DELTA(N2lvl,mxco,mnp,Nsps)
    real(r8) :: DH, DT_SCRPT_DNP(N2lvl,mxco,mnp), DTAUDA(Nlvl,mxco)
    real(r8) :: ETA(Nlvl,mxco)
    integer(i4) :: FFT_INDEX(maxfft)
    real(r8) :: FFT_PRESS(maxfft)
    real(r8) :: FRQ
    integer(i4) :: H_I
    real(r8) :: H_TAN
    integer(i4) :: HT_PTR(Nsps)
    integer(i4) :: I, ICH, IC1, IC2, IHI, ILO, INDXL, INDXR
    real(r8) :: I_RAW(Nlvl), I_STAR_ALL(max_no_pointings)
    integer(i4) :: IS
    integer(i4) :: JCH, JTAN
    integer(i4) :: K
    ...
    real(r8) :: S, SCALE_FACTOR(max_no_pointings)
    integer(i4) :: SPS_COEF_LOOP(2,Nlvl,Nsps), SPS_I, SV_I
    real(r8) :: T, T_SCRIPT(N2lvl), TANX, TAU(N2lvl)
!
...
!
    tau(1:Nlvl) = 0.0
    TAU(1:no_t) = dble(geophysic(t_indx)%basis_peaks(1:no_t))
    Call get_eta(conv_press,TAU,no_conv_hts,no_t,Nlvl,Eta)
!
...
!
     ...
     &         mdb_freq,mnp,mnf,cs_mnf,no_freqs_f,no_phi_f,phi_basis_f, &
...
!
     ...
     &             phi_path(1:,k),dHdz_path(1:,k),dh_dt_path(1:,1:,1:,k),  &
     &             Kgp,mnp,mdb_freq,mdb_pres,mdb_temp,mr_f,mr_g,           &
     &             no_coeffs_f,no_freqs_f,no_phi_f,no_phi_t,no_t,N_lvls,   &
     &             n_sps,phi_basis_f,k,ref_corr(1:,k),sps_tbl,t_phi_basis, &
     &             tau,t_script,dt_scrpt_dnp,mnf,cs_mnf,ilo,ihi,IndxR,     &
     &             IndxL,path_brkpt(1:,k),K_star_geophys,Ier)
              if (Ier /= 0) Return
!
            end if
          end if
...
!
          Ier = Ich           ! DEBUG (for printing in i_and_k_star)
...
!
            if (ich == 61) then
              ...
              write(*,'(''pfa_star'',a1,i2,''n'')') char(92),is
              write(*,'(6(1x,1pg12.6))') (i_star_all(k),k=1,is)
              ier = 5000                    ! ** DEBUG
              Return                        ! ** DEBUG
            end if
