module READ_UIF_M
  ...
  use L2PCDim, only: NLVL, NSPS, MAX_NO_PHI
  use MACHINE, only: IO_ERROR
  use MLSCommon, only: I4, R4, R8
  use D_BRKLINE_M, only: BRKLINE
...
!
 ...
 &           CONV_HTS, NO_CONV_HTS, FFT_PTS, CH1, CH2,                     &
 ...
 &           SPECT_INDEX, L2PC_REC_LENGTH, IER )
    ...
    type (spectro_param), intent(out) :: SPECTROSCOPIC(*)

    type (spectro_param) :: zero_spect

    integer(i4), intent(out) :: NO_GEOM, NO_GEOPHYS, NO_ATMOS, NO_CONV_HTS, &
                 FFT_PTS, CH1, CH2, PFA, NO_PFA_CH, PFA_CH(*), NO_FILT_PTS, &
                 NO_INT_FRQS(*), P_INDX(*), CONV_INDX(*), ATMOS_INDEX(*),   &
                 GEOM_INDEX(*), GEOPHYS_INDEX(*), SPECT_ATMOS(*), N_LVLS,   &
                 SPECT_INDEX(*), L2PC_REC_LENGTH, NO_SPECTRO, SI, IER

    integer(i4), intent(in) :: L2PC_LU

    character(len=*), intent(out) :: INDIR, FND, AAAP

    real(r8), intent(out) :: CONV_HTS(*), ACC(*), TH(*)

    character, intent(out) :: PRIMAG

    real(r8), intent(out) :: C_YAW, S_YAW, C_ROLL, S_ROLL, C_PITCH, S_PITCH, &
              ELEV_183, ELEV_205, AZIM_183, AZIM_205, AZIM_REF, GEOCSRAD
...
!
    ...
    integer(i4) :: I, II, IZ, J, JJ, K, KK, L, Spectag

    character(len=100) :: LINE
    character(len=80) :: MDBD
    character(len=1) :: CA

    integer(i4) :: NC, NO_SPS
...
!
!  Define a "zero" (initialized) spectroscopic record.
!
    zero_spect%type = '@'
    zero_spect%name(1:) = ' '
    zero_spect%spectag = -1
    ...
    do j = 1, max_no_elmnts_per_sv_component+2
      zero_spect%zeta_basis(j) = 0.0
    end do
!
    iz = 3 * Nsps
    do j = 1, iz
      spectroscopic(j) = zero_spect
    end do
!
    iz = 0
    do while (iz == 0)
...
!
        ...
        do i = 1, no_geom
          ...
          Line(1:k) = ' '
          Call Leftj(Line)
          Read (Line,*,iostat=iz) (geometric(i)%der_calc(j),j=1,6),r
          if (iz /= 0) goto 99
      ...
      else if(Line(1:16) == 'NO_SPECTROSCOPIC') then
        Read(Uu,*,iostat=iz) no_spectro
        if(iz /= 0) goto 99
      else if(Line(1:13) =='SPECTROSCOPIC') then
        if(no_spectro < 1) then
          iz = -6
          Line(1:) = ' '
          Line = 'NO_SPECTROSCOPIC is missing or misplaced ..'
          goto 99
        else if(band < 1) then
          iz = -6
          Line(1:) = ' '
          Line='CHANNELS_RANGE must be given before SPECTROSCOPIC ..'
          goto 99
        endif
        pband = no_spectro
        no_spectro = 0
        do i = 1, pband
          ...
          Line(1:k) = ' '
          Call Leftj(Line)
          Read(Line,*,iostat=iz) Spectag,kk
          if(iz /= 0) goto 99
          if(kk.lt.1) then
            iz = -6
            Line(1:) = ' '
            Line = 'Number of SPECTROSCOPIC_Types is < 1'
            goto 99
          endif
          j = no_spectro + kk
          if(j.gt.3*Nsps) then
            iz = -6
            Line(1:) = ' '
            Line = 'NO_SPECTROSCOPIC too large, must be <= 3*Nsps'
            goto 99
          endif
          do k = 1, kk
            no_spectro = no_spectro + 1
            spectroscopic(no_spectro)%spectag = Spectag
            Line(1:) = ' '
            Read(Uu,'(A)',iostat=iz) Line
            if(iz /= 0) goto 99
            Call Leftj(Line)
            CA = Line(1:1)
            spectroscopic(no_spectro)%type = CA
            An1(1:) = ' '
            An1 = Name(1:l)//'_'//CA
            spectroscopic(no_spectro)%name = An1
            j = Index(Line,'.f.')
            if(j.lt.1) j = Index(Line,'.t.')
            if(j.gt.0) then
              Read(Line(j:),*,iostat=iz) (do_der(ii),ii=1,6)
              if(iz /= 0) goto 99
            else
             iz = -6
             Line(1:) = ' '
             Line = 'SPECTROSCOPIC input in error: No Flags !'
             goto 99
            endif
            do j = 1, 6
              spectroscopic(no_spectro)%der_calc(j) = do_der(j)
            end do
            jj = 0
            do while(jj.lt.2)
              Read(Uu,'(A)',iostat=iz) Line
              ...
              Call Leftj(Line)
              j = Index(Line,'PHI')
              if(j.lt.1) j = Index(Line,'ZETA')
              if(j.lt.1) then
               iz = -6
               Line(1:) = ' '
               Line = 'SPECTROSCOPIC input in error: No Zeta/Phi base'
               goto 99
              endif
              An1(1:) = ' '
              j = Index(Line,',')
              if(j.lt.1) j = Index(Line,' ')
              An1 = Line(1:j-1)
              Line(1:j) = ' '
              Call Leftj(Line)
              Read(Line,*,iostat=iz) L
              if(iz /= 0) goto 99
              if(An1(1:4) == 'ZETA') then
                spectroscopic(no_spectro)%no_zeta_values = L
                Read(Uu,*,iostat=iz) &
     &               (spectroscopic(no_spectro)%zeta_basis(j),j=1,L)
              else
                spectroscopic(no_spectro)%no_phi_values = L
                Read(Uu,*,iostat=iz) &
     &               (spectroscopic(no_spectro)%phi_basis(j),j=1,L)
              endif
              if(iz /= 0) goto 99
              jj = jj + 1
            end do
...
!
    if(no_spectro < 1) goto 42
!
!  Compose Spectroscopic information:
!
    do 35 i = 1, no_spectro
!
!  *** Special code for the PC, ONE band  capability only..
!
      if(.not.spectroscopic(i)%der_calc(band)) then
        spectroscopic(i)%spectag = -1
        goto 35
      endif
!
      Spectag = spectroscopic(i)%spectag
      do k = 1, no_atmos
        jj = atmospheric(k)%spectag
        if(jj == Spectag) then
          do j = 1, 6
            if(.not.atmospheric(k)%fwd_calc(j)) then
              if(spectroscopic(i)%der_calc(j)) then
                l = LEN_TRIM(spectroscopic(i)%name)
                CA = spectroscopic(i)%name(l:l)
                Print 903,atmospheric(k)%name,CA,j,CA
                spectroscopic(i)%spectag = -1
                goto 35
              endif
            endif
...
!
 35 continue
!
 903  format('** Warning: Specie:',A,/,3x,      &
     & 'Requesting Spectroscopic derivative for: ',A,' on band:',i2,/, &
     & 3x,'But Specie itself is NOT included in that band !',/, &
     & 3x,'Request for spectroscopic derivative (',A,') is ignored ..')
!
    k = 0
    do i = 1, no_spectro
      if(spectroscopic(i)%spectag > 0) then
        k = k + 1
        if(i > k) spectroscopic(k) = spectroscopic(i)
      endif
    end do
!
    no_spectro = k
    if(no_spectro < 1) then
      Print *,'** Warning: NO Spectroscopic derivatives !'
      goto 42
    endif
!
    do k = 1, no_atmos
