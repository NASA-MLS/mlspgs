...
!
    ...
    real(r8), intent(in) :: CONV_TEMP(*) ! not used
    real(r8), intent(in) :: PTG_HTS(*)
    real(r8), intent(in) :: PTG_ANGLES(*)
    real(r8), intent(in) :: DX_DT(Nlvl,*)
    real(r8), intent(in) :: D2X_DXDT(Nlvl,*)
    integer(i4), intent(in) :: BAND
    real(r8), intent(in) :: CENTER_ANGLE
    integer(i4), intent(in) :: FFT_PTS
    real(r8), intent(out) :: FFT_PRESS(2**fft_pts)
    real(r8), intent(in) :: I_RAW(*)
    character(len=*), intent(in) :: IN_DIR, Aaap
    real(r4), intent(in) :: K_STAR_GEOPHYS(Nlvl,mxco,mnp,*)
    real(r4), intent(in) :: K_STAR_ATMOS(Nlvl,mxco,mnp,*)
    real(r8), intent(in) :: A_GRID(*)
    real(r8), intent(in) :: T_GRID(*)
    real(r8), intent(in) :: Z_GRID(*)
...
!
    fft_angles(1:no_conv_hts) = ptg_angles(1:no_conv_hts)
...
!
    j = ptg_press%no_lin_values
    Call Cspline_der(fft_press,PtP,Rad,i_star_all,       &
   &                  term,k,j)
!
...
!
          Call Lintrp(conv_press,PtP,ptg_angles,         &
...
! Get X^63
!
    Call Lintrp(conv_press,PtP,ptg_angles,fft_angles,    &
   &            no_conv_hts,j)
!
! Get dI/dX at 63
!
