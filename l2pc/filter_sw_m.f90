...
Integer(i4), INTENT(OUT) :: nda,nsa,jcos,numd,lf,ier
...
INTEGER(i4) :: i,j,k,ns,ne,ndeg,io

CHARACTER (LEN=80) :: fln
CHARACTER (LEN=2) :: api

NAMELIST/in/area,xlhs,xrhs,ne,pl,pr,cel,cer,ndeg,pcoeff,ns,scoeff
...
Integer(i4), save :: nda,nsa,jcos,numd
...
! Begin code:
  ...
  if(PRESENT(ich) .and. PRESENT(fdir) .and. PRESENT(prim)) then
    ...
    Call load_filter(ich,ier,fdir,prim,lf,ael,aer,apc,asc, &
                     fa,x1,xl,xr,xn,nda,nsa,jcos,numd)
    if(ier /= 0) Stop
    a1 = x1
    an = xn
    sf = fa
