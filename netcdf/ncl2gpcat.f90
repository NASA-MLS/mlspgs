...
!=================================
program ncl2gpcat ! Copy L2GPData from one NetCDF4 to another
!=================================
...
! This program catenates L2GPData files as netcdf4
!
...
! 
  ...
  type Options_T
    ...
    character(len=255) ::  outputFile    = 'default.nc4'  ! output filename                 
  ...
  do      ! Loop over filenames
     ...
     if ( n_filenames == 1 ) options%inputFile = filename
     if ( n_filenames == 2 ) then
       options%outputFile = filename
       exit
     endif
   enddo
   call OutputNamedValue( 'input L2GP File',  trim(options%inputFile), &
     & options='--Headline' )
   call OutputNamedValue( 'output L2GP File',  trim(options%outputFile), &
     & options='--Headline' )
   call catenate_one_file( trim(filename), options )
   call resumeOutput
...
!------------------------- print_help ---------------------
  ...
  ! Print brief but helpful message
      write (*,*) &
      & 'ncl2gpcat: Copy swaths from one NetCDF4-formated' &
      & // 'L2GP file to another.' &
      & // 'Usage: ncl2gpcat [options] [hdfeos_filenames]'
      write (*,*) &
  ...
  subroutine Defeat(msg)
   ...
   subroutine catenate_one_file ( filename, options )
    ...
    enddo
   end subroutine catenate_one_file
