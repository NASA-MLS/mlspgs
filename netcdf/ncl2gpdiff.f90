...
!=================================
program ncl2gpdiff ! Diff L2GPData from two NetCDF4 files
!=================================
   ...
   use MLSMessageModule, only: MLSMessageConfig, MLSMSG_Error, MLSMSG_Warning, &
     & MLSMessage
   use MLSNetCDF4, only: mls_InqSwath, MLS_SwWrattr
...
! This program diffs L2GPData files from netcdf4
!
...
! 
  ...
  character(len=255)              :: NC4filename1          ! filename
  character(len=255)              :: NC4filename2          ! filename
  integer                         :: n_filenames
  ...
  do      ! Loop over filenames
     ...
     if ( n_filenames == 1 ) NC4FileName1 = filename
     if ( n_filenames == 2 ) then
       NC4FileName2 = filename
       exit
     endif
...
!      call resumeOutput
  ...
  call diff_two_files ( NC4FileName1, NC4FileName2, options )
...
!------------------------- print_help ---------------------
  ...
  ! Print brief but helpful message
      write (*,*) &
      & 'Usage: ncl2gpdiff [options] hdfeos_file netcdf_file'
      write (*,*) &
   ...
   subroutine diff_two_files ( NC4FileName1, NC4FileName2, options )
    ...
    ! Dummy args
    character(len=*), intent(in)         :: NC4FileName1 ! file 1
    character(len=*), intent(in)         :: NC4FileName2 ! file 2
    type ( Options_T )                   :: options
    ! Local variables
    ...
    type(MLSFile_T)                      :: MLSFile1
    type(MLSFile_T)                      :: MLSFile2
    integer                              :: noSwaths
    integer                              :: status
    ...
    ! Get swath list
    noSwaths = mls_InqSwath ( NC4FileName1, SwathList, listSize )
    if ( options%verbose ) then
      call output('swaths in ' // trim(NC4FileName1), advance='yes')
      call dump(trim(swathList))
      ! return
    endif
    ! Executable code
    status = InitializeMLSFile ( MLSFile1, type=l_swath, access=DFACC_READ, &
     & name=NC4FileName1, HDFVersion=HDFVERSION_5 )
    status = InitializeMLSFile ( MLSFile2, type=l_netcdf4, access=DFACC_READ, &
     & name=NC4FileName2 )
    call Dump ( MLSFile1 )
    call Dump ( MLSFile2 )
    ...
    do i = 1, noSwaths
      ...
      ! Allocate and fill l2gp
      print *, 'Reading swath from file: ', trim(NC4FileName1)
      call readNC4Swath ( trim(NC4FileName1), trim(swath), l2gp1 )
      print *, 'Min, Max (1st swath) ', minval(l2gp1%l2gpvalue), maxval(l2gp1%l2gpvalue)
      print *, 'shape (he5 swath) ', shape(l2gp1%l2gpvalue)
      print *, 'Reading 2nd swath from file: ', trim(NC4FileName2)
      call readNC4Swath ( trim(NC4FileName2), trim(swath), l2gp2 )
      print *, 'Min, Max (nc4 swath) ', minval(l2gp2%l2gpvalue), maxval(l2gp2%l2gpvalue)
      print *, 'shape (nc4 swath) ', shape(l2gp2%l2gpvalue)
      ! Diff the swaths
...
!==================
end program ncl2gpdiff
!==================

! $Log$
! Revision 1.2  2022/11/17 22:16:46  pwagner
! Fixed bugs; removed unused stuff
!
