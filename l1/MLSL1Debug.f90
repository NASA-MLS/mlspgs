...
!-
  ...
  ! take out once this has been checked in.
  ! 
  ! I can't `use' Calibration to get max_cal_index because Calibration `use's
  ! me. max_cal_index is a parameter == WinMAFs*150-1, but WinMAFs is also a
  ! parameter in Calibration. I need something to dimension these arrays, so I'm
  ! going to assume that WinMAFs will never be > 20

  ! INTEGER, PARAMETER:: maxsize=20*150-1
  ! TYPE MLSL1Debug_ComVec_Rec_T
  !    SEQUENCE 
  !    REAL(r8) :: weight(0:maxsize) ! 
  !    REAL(r8) :: time(0:maxsize)
  !    REAL(r8) :: comVec(0:MaxMIFS-1,0:maxsize)
  !    REAL(r8) :: errmul(0:MaxMIFs)
  !    INTEGER  :: qual(0:maxsize)
  !    INTEGER  :: altIndex ! == -1 (type != 'LS', 1 - maxAlts)
  !    CHARACTER(len=2) :: recType ! 'LS','L','T'
  !    character(len=1) :: seq(maxsize)
  !    character(len=1) :: spare
  ! END TYPE MLSL1Debug_ComVec_Rec_T
  
  ! type (MLSL1Debug_ComVec_Rec_T) ComVecRec
  ...
  TYPE (MLSL1DebugCntrlLogicals_T) DebugControl
  TYPE (MLSL1DebugFileInfo_T) ComVecsFileInfo, RadiancesFileInfo


  LOGICAL :: initialized=.FALSE., SnoopComVec=.FALSE., SnoopRadiances=.FALSE.

  INTEGER :: altIndex, dbMAF, dbMIF, comVecFileLun=-1
  REAL(r8) dbTAI93
  CHARACTER(len=2)comVecRecType ! either 'LS','S', or 'T'. 

  ! For use in Calibration::*ComVecs routine debugging
  !CHARACTER(len=512) ComVecsFile, RadiancesFile
