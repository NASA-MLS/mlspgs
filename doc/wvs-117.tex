...
\newcommand{\docname}{wvs-117r1}
\newcommand{\docdate}{21 February 2014}
...
Several new facilities have been added to the configuration file.
...
\item \emph{variable := expr}, where \emph{variable} is a name, can appear
  ...
  element is a variable that has an array value. Once a variable has a
  type, it cannot be assigned a different type, and if it is numeric its
  units cannot change, but it can change between array and scalar.
...
\item An IF construct, with the same syntax as in Fortran, can appear
  ...
  logical value; it can depend upon the value of a variable.
...
\item A SELECT CASE construct, with the same syntax as in Fortran (except
  ...
  expressions in the SELECT CASE and CASE statements must be scalars; they
  can depend upon the values of variables.
...
\item A DO construct, of two forms.  The syntax is {\tt DO}
  ...
  integer type, and the semantics are the same as for the Fortran DO
  construct.  If there is only one expression, it can be of any type, and
  must be an array.  The \emph{variable} is assigned elements of the
  value of \emph{expr}.
...
\item A CYCLE statement of the form {\tt CYCLE} \emph{do-label}, where
  \emph{do-label} is the label of a DO or WHILE construct.

\item An EXIT statement of the form {\tt EXIT} \emph{do-label}, where
  \emph{do-label} is the label of a DO, IF, SELECT CASE, or WHILE
  construct.
...
\item {\tt difference ( a, b )} computes the symmetric difference of the
  ...
  {\tt union ( without ( a, b ), without ( b, a ) )}.
...
I have not changed {\tt mlsl1} or {\tt mlsl3} to handle the IF and SELECT
case constructs.  Changes to {\tt mlsl1} should be simple, but since {\tt
mlsl3} does not use the abstract syntax tree directly, it would be
necessary also to change the alternative representation used in {\tt
mlsl3}.
