...
\newcommand{\docname}{\bf wvs-076r3}
\newcommand{\docdate}{29 April 2008}
...
MIF $\times$ MAF quantity; the $\phi$ values for the MIFs for the
hypothetical reference MAF should all be set to the same value, probably
using a {\tt forge} command in the L2CF.

\item The equivalent earth radius $\R$ is computed from $\phi_i$ and the
orbit inclination using Equations (5.3) and (5.21) from the 19 August 2004
ATBD JPL D-18130.
...
phi} and {\tt surfs} fields of the output structure.
...
\item The height $H_{jk}$ of $S_{jk}$ is calculated by interpolating in
{\tt h\_GLgrid}.  Since $H_{jk}$ is a breakpoint, its height as
calculated by {\tt metrics} should be essentially the same.

\item For each $\phi_i$ and $\phi_j$ the angle $\phi_{ij} = \phi_j -
\phi_i$ is the angle in the orbital plane between the hypothetical
reference MAF and the scattering point, as seen from the center of the
equivalent circular earth tangent to the true earth at the surface point
below the tangent point on the hypothetical reference MAF.
...
$|\{\phi_i\}| \leq 3$ probably.  $\{P_{mn}(\chi_l)\}$ was calculated with
...
\approx 7$,  $|\{\overline{D}^s_{ijkmnqr}\}| \approx 3.5\times 10^8$, or
