...
{\parskip 5pt
The abstract syntax tree is represented in the {\tt tree} module.

It is stored using {\bf cdr code}.  Sibling vertices are stored in
consecutive elements of {\tt the\_tree} (a private array in the {\tt tree}
module).  Their parent vertex is stored later.  It is represented by a
tree vertex kind, a tree node index (in {\tt tree\_types}), the location
of the first son, and the number of sons.
...
%\vspace*{0.38in}
\includegraphics[scale=0.5,angle=270]{wvs-147-6}
...
\vspace*{1in}
\includegraphics[bb=17 71 466 716,scale=0.50,angle=90]{tree-1}
...
{\tt root} is the root of the subtree; {\tt first\_section} is the subtree
index of the next son to process.

Get the next son and update the state (which one to do next):
...
{\tt root} is the root of the subtree.  Its sons are either specifications
or parameter assignments.  (In MLSSignals we know they're all
specifications.)  Get the next son:
...
{\tt {\bf call} get\_label\_and\_spec ( soon, name, key )\\
...
\hspace*{0.1in}son = subtree(j,key)\\
