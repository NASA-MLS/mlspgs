...
\newcommand{\docname}{\bf wvs-070r5}
\newcommand{\docdate}{12 August 2014}
...
From: \>Van Snyder\\
Reference: \>Cloud ATBD JPL D-19299 (4 June 2004), wvs-067, wvs-068
\end{tabbing}
...
C(r) = \frac12 \int_0^\pi p_0(\theta,r)\, \sin\theta\,\d\theta = \sum \left(
...
The quantities $G_{ij}$ are evaluated below, giving a simple explicit
representation for $C(r)$.

Thus, using this form for $C(r)$, the integrated phase function in
Equation (\ref{six}) and its derivative w.r.t.\ IWC are no longer double
integrals ($p_0(\theta,r)$ does not depend upon IWC).
...
This simplification does not occur for $S_1^2$ or $S_2^2$ separately, but
we evaluate $p_0(\theta,r)$ in the form of Equation (\ref{two}), not
Equation (\ref{twelve}), so it does not matter.

Observing that $\xi_s(r) = \frac2{\chi^2}\, C(r)$ (see wvs-067) and $\chi
= \frac{2\pi r}{\lambda}$, the integral in Equation (\ref{six}) simplifies
to
...
% $Log$
% Revision 1.3  2013/01/17 21:08:21  vsnyder
% Add CVS Log line
%
