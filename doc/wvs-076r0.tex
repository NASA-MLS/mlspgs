...
\newcommand{\docname}{\bf wvs-076}
\newcommand{\docdate}{10 April 2008}
...
using a {\tt forge} command.

\item The latitude and equivalent earth radius $R^\oplus_i$ for each
$\Pi_i$ are computed in the usual way.
...
and $\{\zeta_k\} \subseteq Z$, else radiative transfer can't stop at
$S_{jk}$.
...
critical angle $\chi^c_{ijk} = \phi_{ij} - \cos^{-1}\left( R^\oplus_i /
H_{jk} \right)$.

The quantity $R^\oplus_i$ is not precisely the correct value; what is
really wanted is the radius of the equivalent circular earth at the point
of reflection of the ray from the earth's surface, which is not
necessarily the same as the radius to the point on the surface below the
limb ray tangent point.  For small values of $\phi_{ij}$, $R^\oplus_i$
should be sufficiently close to the correct value.
...
\item\label{final} The sets of temperatures $T = \{T_m\}$ and ice water
content values IWC = \{IWC$_n$\} are the same at every scattering point.
They are taken from the tables of Mie phase functions.  At each $S_{jk}$
and for each $T_m$, IWC$_n$, and species $s$, the radiances $I_{ijkl}$
and derivatives $D^s_{ijklp}$ calculated in step \ref{xfer} are convolved
