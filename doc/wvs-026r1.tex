...
\newcommand{\docname}{\bf wvs-026r1}
\newcommand{\docdate}{14 October 2005}
...
The frequency-averaged $\tau^s_{in}$ in each
channel at each point along the path is
...
and substituting $\overline{\tau^s_{ic}}$,
...
\approx \sum_{n=1}^{N_f} \phi_{nc} \Delta \nu_{nc} \sum_{i=1}^{N_p}
 \left(
  ...
  \right) \tau^s_{in}
 -
 \sum_{i=1}^{N_p} \overline{\tau^s_{ic}} \tau^w_{ic}
...
is a PFA correction.  The second term can be evaluated within the frequency
loop, thereby not needing to provide a $c$ dimension for $\frac{\partial
\delta^w_{jc}}{\partial x_k}$.

The frequency-averaged radiance can be gotten from Equation (\ref{tau}) by
exchanging the order of summation in Equation (\ref{combined}):

\begin{equation}
I_c \approx \sum_{i=1}^{N_p} \tau^w_{ic} \overline{\tau^s_{ic}}\,.
\end{equation}
