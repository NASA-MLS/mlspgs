...
\newcommand{\docname}{wvs-146r9}
\newcommand{\docdate}{17 March 2020}
...
be the vector to a point on its surface, where $\lambda$ is longitude,
$\theta$ is reduced latitude,\footnote{Called the \emph{eccentric anomaly}
in astronomy.} and $e$ is the eccentricity of the oblate
spheroid.\footnote{The eccentricity $e$ is not to be confused with
...
surface is an ellipse with semi-major axis is $a$, and semi-minor axis
...
Substituting $x = u - u_c$ and $y = v - v_c$, and solving for $u_c$ and
$v_c$ to make the coefficients of the terms linear in $u$ and $v$ zero,
Equation (\ref{Normal-rot}) becomes
%
...
\pageref{geometric-figure}, and using $|\hat{n}| = 1$, $C = \cos\phi$ and
$\sqrt{A^2+B^2} = \sqrt{1-C^2} = \sin\phi$.
...
$\angle(\vec{V},\vec{G}) = \phi - \gamma = \cos^{-1} (\mathbf{g} \cdot
\hat{V})$.
