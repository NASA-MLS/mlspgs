...
\newcommand{\docname}{wvs-146r3}
\newcommand{\docdate}{19 January 2020}
...
normal section at azimuth $\alpha$, in a local $(\xi',\eta')$ co\"ordinate
system with its origin at $(\lambda,\phi)$, is
%
...
Let $\vec{T} = [x_t,y_t,z_t]^T$ be the vector to the limb sub-tangent
point, $\vec{G}$ be the outward gradient at the limb sub-tangent point,
and $\vec{V}$ be a vector parallel to the line of sight from the
instrument toward the limb tangent.  Then $\vec{N} = \vec{G} \times
\vec{V}$ is normal to the plane containing the line of sight and the
outward normal at the limb tangent point.  Let $\hat{n} =
\frac{\vec{N}}{|\vec{N}|} = [ A, B, C ]^T$ be the unit normal to that
plane.  Then the equation of the plane containing the line of sight, the
outward normal at the limb tangent point, and the limb tangent point is %
\begin{equation}\label{z-plane} A ( x - x_t ) + B ( y - y_t ) + C ( z -
z_t ) = 0 \text{ or } A x + B y + C z = d \,\text{, where } d = \hat{n}
\cdot \vec{T}\,. \end{equation}

This plane is a normal section.  Substituting
%
\begin{equation}
z = \frac{d - A x - B y}C \,.
\end{equation}
...
where $W^2 = A^2 + B^2 + F^2 = 1 - e^2 C^2$, from which the semi-major
axis and eccentricity of the ellipse in the normal section are
%
\begin{equation}
a'^2 = a^2-\frac{d^2}{W^2} \text{ and }
\epsilon^2 = 1 - \frac{F^2}{W^2} = \frac{A^2+B^2}{W^2} \,.
\end{equation}

Substituting $\psi = \omega = \tan^{-1} \frac{A}B$ into Equation
(\ref{psi-rot}) gives the radius of curvature at the sub-tangent point,
%
...
From the figure in section \ref{geometric}, $d = \hat{n} \cdot \vec{T} =
\zeta \cos \phi$ is the distance from the center to $\mathcal{L}$. 
Substituting $\zeta = z_t \frac{e^2}{1-e^2}$ from Equation (\ref{zeta})
and $z_t=a\,\frac{(1-e^2) \sin \phi}{\sqrt{1-e^2 \sin^2 \phi}}$ from
Equation (\ref{nine}),
%
\begin{equation}
d = \hat{n} \cdot \vec{T}
...
gradient at that point is $\vec{G} = \nabla \Sigma = [ 2 x, 2 y,
\frac{2z}{1-e^2} ]^T$.  Then using Equation (\ref{ellipsoid}),
...
Using Equation (\ref{first}),

\begin{equation}\label{VG}
\frac{\vec{V} \cdot \vec{G}}{|\vec{V}| \, |\vec{G}|} =
 \left( 1 + \frac{e^4 \sin^2 \theta \cos^2 \theta}{1-e^2}
 \right)^{-\frac12}
=
 \left( 1 + \frac{e^4 \sin^2 2 \theta}{2(1-e^2)} \right)^{-\frac12}
\,.
\end{equation}

Using $\rho^2 = x^2 + y^2$ and solving Equation (\ref{first}) for $\sin
\theta = \frac{z}{a\sqrt{1-e^2}}$ and $\cos \theta = \frac{\rho}a$, this
may be expressed in Cartesian co\"ordinates as

\begin{equation}\begin{split}
  \frac{\vec{V} \cdot \vec{G}}{|\vec{V}| \, |\vec{G}|}
 = \,&
  \left( 1 + \frac{e^4 \rho^2 z^2}{a^4(1-e^2)^2} \right)^{-\frac12}
 = \left( 1 + \frac{e^4 \rho^2 z^2}{c^4} \right)^{-\frac12}
% =  a^2 \frac{1-e^2}{\sqrt{a^4(1-e^2)^2 + e^4 \rho^2 z^2}}
  \\
 = \,&
  \left( 1 + \frac{e^4 \rho^2 ( a^2 - \rho^2) }
                  { a^4 ( 1-e^2 )} \right)^{-\frac12}
 = \left( 1 + \frac{e^4 \rho^2 ( a^2 - \rho^2)}{a^2 c^2} \right)^{-\frac12}
% =  a^2 \, \sqrt{\frac{1-e^2}{a^4(1-e^2) + e^4 \rho^2 ( a^2 - \rho^2 )}}
  \,. \\
\end{split}\end{equation}

From the figure in section \ref{geometric}, $\angle(\vec{V},\vec{G}) =
\phi - \gamma = \cos^{-1} \frac{\vec{V} \cdot \vec{G}}{|\vec{V}| \, |\vec{G}|}$.
Then using Equation (\ref{tangents}),

\begin{equation}\begin{split}
\frac{\vec{V} \cdot \vec{G}}{|\vec{V}| \, |\vec{G}|} = \,&
\cos(\phi - \gamma) = \cos \phi \cos \gamma + \sin \phi \sin \gamma \\
 = \,& \frac{a^2 \cos^2 \phi + c^2 \sin^2 \phi}
            {\sqrt{a^4 \cos^2 \phi + c^4 \sin^2 \phi}}
 = \frac{1 - e^2 \sin^2 \phi}
        {\sqrt{\cos^2 \phi + (1-e^2)^2 \sin^2 \phi}}
 = \frac{1 - e^2 \sin^2 \phi}
        {\sqrt{ 1 + e^2 ( e^2-2) \sin^2 \phi}} \\
 = \,& \frac{a^2 \sin^2 \gamma + c^2 \cos^2 \gamma}
            {\sqrt{a^4 \sin^2 \gamma} + c^4 \cos^2 \gamma}
 = \frac{1 - e^2 \cos^2 \gamma}
        {\sqrt{ \sin^2 \gamma + (1-e^2)^2 \cos^2 \gamma}}
 = \frac{1 - e^2 \cos^2 \gamma}
        {\sqrt{ 1 + e^2 ( e^2-2) \cos^2 \gamma}} \\
 = \,& \frac{a\,c}
            {\sqrt{(a^2 \cos^2 \theta + c^2 \sin^2 \theta)
                   (c^2 \cos^2 \theta + a^2 \cos^2 \theta)}} \\
 = \,& \sqrt{\frac{1-e^2}
                  {( 1 - e^2 \sin^2 \theta )
                   ( 1 - e^2 \cos^2 \theta )}}
 = \left( 1 + \frac{e^4 \sin^2 2 \theta}{2(1-e^2)} \right)^{-\frac12}
