...
\newcommand{\docname}{wvs-146r4}
\newcommand{\docdate}{12 February 2020}
...
$\kappa_1$ and $\kappa_2$, respectively, which are roots of Equation (125)
from {\bf Vector and Tensor Analysis}:
...
\text{d} s^2 = E\, \text{d} \lambda^2 +
               ...
               G \, \text{d} \theta^2
             = \sum_{ij} g_{ij} \text{d}x^i \text{d}x^j \,.
\end{equation}

where $g_{ij}$ is the metric tensor defined by Equation (35) on page 122
of {\bf The Absolute Differential Calculus}, \emph{viz}.

\begin{equation}
g_{ij} = \sum_n
\frac{\partial y_n}{\partial x^i} \frac{\partial y_n}{\partial x^j} \,.
\end{equation}

For an oblate spheroid using reduced latitude $\theta$, where $\mathbf{y}
= \mathbf{r}$ and $\{x_1,x_2\} = \{\lambda, \theta\}$,
...
% \begin{equation}\begin{split}
% g = \,& \left[ \begin{array}{ll} E & F \\[6pt] F & G \\ \end{array}\right]
%   = \left[ \begin{array}{ll}
%         \frac{\partial \mathbf{r}}{\partial \lambda} \cdot
%         \frac{\partial \mathbf{r}}{\partial \lambda} &
%         \frac{\partial \mathbf{r}}{\partial \lambda} \cdot
%         \frac{\partial \mathbf{r}}{\partial \theta} \\[6pt]
%         \frac{\partial \mathbf{r}}{\partial \lambda} \cdot
%         \frac{\partial \mathbf{r}}{\partial \theta}  &
%         \frac{\partial \mathbf{r}}{\partial \theta} \cdot
%         \frac{\partial \mathbf{r}}{\partial \theta} \\
%     \end{array} \right]
%   = \left[ \begin{array}{ll} a^2 \cos^2 \theta & 0 \\[6pt]
%                              0 & a^2 \sin^2 \theta + c^2 \cos^2 \theta
%     \end{array} \right] \\[6pt]
%   = \,&
%     \left[ \begin{array}{ll} a^2 \cos^2 \theta & 0 \\[6pt]
%                              0 & a^2 ( 1-e^2 \cos^2 \theta
%     \end{array} \right] \,.
% \end{split}\end{equation}

$e$, $f$, and $g$ are coefficients of the second fundamental
form:\footnote{Within this section, section \ref{Gaussian}, and section
\ref{Mean}, $e$ is not the eccentricity of the oblate spheroid.}
%
\begin{equation}
...
- e^2 \cos^2\theta}{\sqrt{1-e^2}}$ is the geometric mean of the principal
radii of curvature.
...
normal to the surface.\footnote{B. R. Bowring, \emph{Notes on the
curvature in the prime vertical section}, {\bf Survey Review 29}, 226
(October 1987) pp 195-196, reproduced in the appendix on page
\pageref{appendix}.}  Using $c \tan \phi = a \tan \theta$ and $c^2 \tan
\phi = a^2 \tan \gamma$ from Equations (\ref{tangents}), the results
derived in Section \ref{geometric} and Equations
...
will intersect the polar axis. The gradient to the ellipsoid at a point $P
= (x,y,z)$ is
%
\begin{equation}
...
Substituting $x = u - u_c$ and $y = v - v_c$, and solving for
$u_c$ and $v_c$ to make the coefficients of the $u$ and $v$
...
Let $T$ be the limb sub-tangent point with geodetic co\"ordinates
$(\lambda,\phi,0)$ and geocentric co\"ordinates $( x_t, y_t, z_t )$,
$\vec{T} = [ x_t, y_t, z_t ]^T$ be the geocentric vector to $T$, $\vec{G}$
be the outward normal at $T$, and $\vec{V}$ be a vector along the line of
sight from the instrument toward the limb tangent at $(\lambda,\phi,h)$. 
Then $\vec{N} = \vec{G} \times \vec{V}$ is normal to the plane containing
$T$, the line of sight, and the outward normal at $T$.  Let $\hat{n} =
\frac{\vec{N}}{|\vec{N}|} = [ A, B, C ]^T$ be the unit normal to that
plane.  From geometry, $\frac{C}{\sqrt{A^2+B^2}} = \cot\phi$.  Because
$|\hat{n}| = 1$, $C = \cos\phi$ and $\sqrt{A^2+B^2} = \sqrt{1-C^2} =
\sin\phi$.

The equation of the plane containing $T$, $\vec{V}$, and $\vec{G}$ is
%
\begin{equation}\label{z-plane} A ( x - x_t ) + B ( y - y_t ) + C ( z -
z_t ) = 0 \text{ or } A x + B y + C z = d = \hat{n}
\cdot \vec{T}\,. \end{equation}
...
eliminate the linear terms, gives
%
\begin{equation}
...
From the figure in section \ref{geometric}, for the prime vertical
section, $d = \hat{n} \cdot \vec{T} = \zeta \cos \phi$ is the distance
from the center to $\mathcal{L}$.  Substituting $\zeta = z_t
\frac{e^2}{1-e^2}$ from Equation (\ref{zeta}) and $z_t=a\,\frac{(1-e^2)
\sin \phi}{\sqrt{1-e^2 \sin^2 \phi}}$ from Equation (\ref{nine}),
%
\begin{equation}
d = \hat{n} \cdot \vec{T}
  = a e^2 \, \frac{\sin\phi \cos\phi}{\sqrt{1-e^2 \sin^2 \phi}} \,.
\end{equation}

Using $a$ = 6378137.0 meters and $e^2 = 6.69437999014\!\times\!10^{-3}$ 
...
%=========================================================================
\section{Relationship between gradient and position vectors}

Let $\vec{V} = [ x, y, z ]^T$ be the position on an ellipsoid.  The
...
%
\begin{equation}
\vec{V} \cdot \vec{G} = 2 \left( x^2 + y^2 + \frac{z^2}{1-e^2}\right)
 = 2 a^2 \,.
\end{equation}

The unit gradient $\frac{\vec{G}}{|\vec{G}|} = \mathbf{n}$, Equation
(\ref{unit-gradient}), is particularly simple in geodetic co\"ordinates.

Using geodetic co\"ordinates, the unit vector $\hat{V}$ parallel to the
position vector becomes

\begin{equation}
\hat{V} = \frac1{\sqrt{\cos^2\phi + (1-e^2)^2 \sin^2\phi}}
\left[ \begin{array}{l}
  \cos\phi \cos\lambda \\
  \cos\phi \sin\lambda \\
  (1-e^2) \sin\phi \\
...
From the figure in section \ref{geometric}, $\angle(\vec{V},\vec{G}) =
\phi - \gamma = \cos^{-1} (\hat{V} \cdot \mathbf{n})$.
...
\tan(\phi - \gamma) =\,& \frac{e^2 \sin\phi \cos\phi}
                    ...
                    = \frac{a}c\, e^2 \sin\theta \cos \theta
                    = \frac{a}{2c}\, e^2 \sin 2 \theta \,.\\
\end{split}\end{equation}

The angle $\phi-\gamma$ between the gradient and geocentric position
vectors is the angle between a profile that is normal to the Earth surface
(i.e., along a line of constant longitude and geodetic latitude), and one
that is along a line of constant geocentric latitude.

Using $e^2 = 6.69437999014\!\times\!10^{-3}$ from WGS84 Table 3.3, the
maximum value of $\phi-\gamma$ is 0\degs1924243011 at $\phi$ =
45\degs09621214, $\gamma$ = 44\degs90378783, $\theta$ = 44\degs99999998. 
At that latitude and an altitude of 100 kilometers, the distance between
points on the two profiles is 335.83562338 meters.

\includegraphics[width=0.48\textwidth,keepaspectratio]{wvs-146-4}
\includegraphics[width=0.48\textwidth,keepaspectratio]{wvs-146-5}
...
\eta = \,& \frac{(1-\epsilon^2)\sin\OP}{\sqrt{1-\epsilon^2\cos^2\OP}} \,, \\
\end{split}\end{equation}

and the unit gradient within the orbit plane, analogously to the
definition of the unit gradient $\mathbf{n}$ to the Earth surface in
...
Then the angle $\Gamma$ between the gradients is give by

\begin{equation}
\cos\Gamma =
\mathbf{n} \cdot \mathbf{n}_p =
\cos\phi ( \cos\lambda \cos\OP +
           \sin\lambda \cos\beta\, \sin\OP ) +
\sin\phi \sin\beta \, \sin\OP \,.
\end{equation}
...
Using Equations (\ref{xyz}) this becomes

\begin{equation}\begin{split}
\tan\lambda = \,& \frac{\eta\cos\beta}{\xi}
            = (1-\epsilon^2)\cos\beta\, \tan\OP
            = \frac{(1-e^2)\cos\beta}{1-e^2\cos^2\beta} \, \tan\OP\\
\tan\phi = \,& \frac{z}{(1-\epsilon^2)\sqrt{x^2+y^2}}
         = \frac{\eta\sin\beta}
                {(1-\epsilon^2)\sqrt{\xi^2 + \eta^2\cos^2\beta}}
         = \frac{\sin\beta \, \sin\OP}
                {\sqrt{\cos^2 \OP +
                            (1-\epsilon^2)^2 \cos^2\beta\sin^2 \OP}} \\[3pt]
         = \,& \frac{( 1-e^2 \cos^2\beta)\, \sin\beta\, \sin\OP}
                    {\sqrt{(1-e^2\cos^2\beta)^2 \cos^2\OP +
                           (1-e^2)^2\cos^2\beta \sin^2\OP}} \,.
\end{split}\end{equation}
...
\sin\phi = \,&
 \frac{( 1-e^2 \cos^2\beta)\, \sin\beta\, \sin\OP}
      {\sqrt{(1-e^2\cos^2\beta)^2 (\sin^2\beta\sin^2\OP + \cos^2\OP) +
             (1-e^2)^2 \cos^2\beta \sin^2\OP}} \\[3pt]
\cos\phi = \,&
 \sqrt{\frac{(1-e^2\cos^2\beta)^2 \cos^2\OP +
             (1-e^2)^2\cos^2\beta \sin^2\OP}
            {(1-e^2\cos^2\beta)^2 (\sin^2\beta\sin^2\OP + \cos^2\OP) +
             (1-e^2)^2 \cos^2\beta \sin^2\OP}} \\[3pt]
\sin\lambda =\,&
 \frac{(1-e^2) \cos\beta \tan\OP}
      {\sqrt{(1-e^2\cos^2\beta)^2 +
             (1-e^2)^2\cos^2\beta \tan^2\OP}} \\[3pt]
\cos\lambda =\,&
 \frac{1-e^2 \cos^2 \beta}
      {\sqrt{(1-e^2\cos^2\beta)^2 +
             (1-e^2)^2\cos^2\beta \tan^2\OP}} \\
\end{split}\end{equation}

The maximum value of $\Gamma$ occurs for $p = \frac{\pi}2$, for which

\begin{equation}\begin{split}
\cos\Gamma = \,& \frac{1 - e^2 \cos^2\beta ( 2 - \cos^2\beta )}
                      {\sqrt{(1-e^2)^2 \cos^2\beta +
                             (1-e^2 \cos^2\beta) \sin^2\beta)}} \\[3pt]
\frac{\d\cos\Gamma}{\d\beta} = \,&
\frac{e^4(1-e^2\cos^2\beta(4-(2+\cos^2\beta))\cos\beta \sin^5\beta}
     {\left((1-e^2)^2 \cos^2\beta +
            (1-e^2 \cos^2\beta) \sin^2\beta)\right)^\frac32} \text{
            and}\\[3pt]
\frac{\d\cos\Gamma}{\d\beta} = \,& 0 \text{ for }
\beta = \sqrt{\frac{2-2e^2-\sqrt{(1-e^2)(4-e^2)}}
                   {2-e^2-\sqrt{(1-e^2)(4-e^2)}}} \,.
\end{split}\end{equation}

The maximum value of $\Gamma$ occurs for $p=\frac{\pi}2$. Using $e^2 =
6.69437999014\!\times\!10^{-3}$ from WGS84 Table 3.3,
$\frac{\d\cos\Gamma}{\d\beta}=0$ for $\beta =$ 59\degs9374951991 $\approx
\frac{\pi}{3.003}$ radians.  At that value of $\beta$,
$\Gamma=$0\degs124930964769.  At an altitude of 100 kilometers, the
horizontal positions on the two gradients are different by 218.045494513
meters.

\begin{minipage}[t]{0.42\textwidth} Angle $\Gamma$ in degrees between
$\vec{G}$ and $\vec{G}_p$ for EOS AURA, which is in an orbit plane
inclined at 98\degs14.  The maximum value of $\Gamma$ is
0\degs0526981133094, and the maximum distance between points on the two
profiles at an altitude of 100 kilometers is 91.9755457161 meters.\\[3pt]
\begin{centering}
...
\begin{centering}
Angle $\Gamma$ in degrees between $\vec{G}$ and $\vec{G}_p$ for\\
$0 \leq \phi \leq \beta \leq \frac{Pi}2$.\\[3pt]

\ifnum \pdfoutput>0 \vspace*{0.1in} \fi
\includegraphics[width=0.95\textwidth,keepaspectratio]{wvs-146-7} \\
\end{centering}
\end{minipage}
