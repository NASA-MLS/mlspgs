...
\newcommand{\docname}{wvs-146r6}
\newcommand{\docdate}{2 March 2020}
...
\theta + \d \theta) - {\bf r}(\lambda, \theta)$, and $\mathbf{g}$ is the
...
%
\begin{equation}
\vec{G} \cdot \vec{V} = 2 \left( x^2 + y^2 + \frac{z^2}{1-e^2}\right)
 = 2 a^2 \,.
\end{equation}
...
From the figure on page \pageref{geometric}, $\angle(\vec{V},\vec{G}) =
\phi - \gamma = \cos^{-1} (\mathbf{g} \cdot \hat{V})$.
...
\tan(\phi - \gamma) =\,& \frac{e^2 \sin\phi \cos\phi}
                    ...
                    = \frac{e^2}{2\sqrt{1-e^2}}\, \sin 2 \theta \,,\\
\end{split}\end{equation}
...
meridional or equatorial.  From the figure on page \pageref{geometric},
the distance of a normal section from the center of the Earth is
%
\begin{equation}
d = \frac{a e^2 \sin\phi \cos\phi}{\sqrt{1-e^2 \sin\phi}} \,.
\end{equation}

The maximum value of $d$ occurs for $\phi$ = 45\degs0340432888.
Using a = 6378137.0 meters from WGS84 Table 3.3, the maximum value of $d$
is 21399.5604406 meters.
...
%
\begin{equation}\begin{split}
G = \,& 1-e^2,\, H = 1-e^2\cos^2\beta,\, E = 1 - \epsilon^2 = \frac{G}H \\
S^2 = \,& E^2 \cos^2\beta\,\sin^2\OP + \cos^2\OP
    = \frac{G^2}{H^2} \cos^2\beta\,\sin^2\OP + \cos^2\OP \\
T^2 = \,& S^2\,H^2 + \sin^2\beta\,\sin^2\OP
    = G^2 \cos^2\beta\,\sin^2\OP + H^2 \cos^2\OP + \sin^2\beta\,\sin^2\OP
    \,, \\
\end{split}\end{equation}
this becomes
\begin{equation}\begin{split}
...
\cos\phi = \,& \frac{S H}T \\
\sin\phi = \,& \frac{\sin\beta\,\sin\OP}T\,. \\
\end{split}\end{equation}

Substituting into Equation (\ref{cosG}), we have
%
\begin{equation}\begin{split}
\cos\Gamma = \,& \frac{H}T \\
\frac{\partial \cos\Gamma}{\partial p} = \,&
\frac{\partial \cos\Gamma}{\partial \beta} = 0
\text{ when }
\OP = \frac\pi2 \text{ and } \beta = \tan^{-1} \sqrt{1-e^2}
 = \cos^{-1}\frac1{\sqrt{2-e^2}}
\text{, giving} \\
\cos\Gamma =\,& \frac{2\sqrt{1-e^2}}{2-e^2} \text{ or }
\tan\Gamma = \frac{e^2}{2\sqrt{1-e^2}}\,,
\end{split}\end{equation}

the same result as Equation (\ref{phi-gamma}). Using $e^2 =
6.69437999014\!\times\!10^{-3}$ from WGS84 Table 3.3, $\beta_0 =$
44\degs9037878494 $\approx \frac{\pi}{4.0086}$ radians and
...
$\mathbf{n}_p \cdot ( \mathbf{g} \times \mathbf{g}_p ) = 0$, where
%
\begin{equation}
