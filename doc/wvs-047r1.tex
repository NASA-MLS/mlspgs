...
\newcommand{\docname}{\bf wvs-047r1}
...
$\zeta$ grid to {\tt ptan} in {\tt phitan}.  This is done in {\tt
Estimate\_tan\_phi} in {\tt FullForwardModel\_m}.

\item\label{hydro} Use the {\tt two\_d\_hydrostatic} routine to calculate
reference arrays on the initial coarse $\zeta$ grid and the temperature
$\phi$ basis.
...
$\zeta$s appear on each side of the tengent, and the current reference
...
\item\label{inner} Calculate the minimum height and its corresponding
$\phi$ for the tangent $\zeta$ using Hermite interpolation in the
reference height array ( see {\tt wvs-046}), or calculate the minimum
$\zeta$ and its corresponding $\phi$ for the tangent height using the
method in {\tt wvs-045}.

\item\label{three} Using the $\phi$s and heights calculated in steps
(\ref{inner}) and (\ref{current}), and inverse interpolation in the
height reference grid, calculate $\zeta$s for temperature basis $\phi$s
(and for the $\phi$ calculated in step (\ref{inner}) if the method from
{\tt wvs-046} is used).  Discard any $\zeta$ and its associated $\phi$ if
...
(\ref{inner}) or (\ref{current}).

\item For each $\phi$ calculated in step (\ref{current}) or (\ref{inner})
and not discarded in step (\ref{three}), calculate a $\phi$ positioned
...
\item\label{augment} Discard any $\zeta$s calculated in step
(\ref{inner}) or (\ref{three}) in a prior iteration, then augment the
$\zeta$ grid using the $\zeta$s newly calculated in step (\ref{zeta}),
giving a new current $\zeta$ grid.  Keep track of whether a new $\zeta$ is
used on both sides of the tangent, or which side if it's only used on one
side.
...
adding $\zeta$s and $\phi$s indefinitely.
...
second one is $\zeta_{\text{min}}$, the third one is its image on the
other side of the tangent, the fifth one arises from a temperature-basis
$\phi$, and the sixth one arises from its image on the other side of the
tangent, while 1, 4, 7, 8, 9, 10 are from the original coarse $\zeta$
grid.  Then on one side of the tangent, the $\zeta$ indices would be 1,
2, 4, 5, 7, 8, 9, 10, while on the other side they would be 1, 3, 4, 6,
7, 8, 9, 10.  The {\tt h\_ref} array would have 10 rows, but for the
calculations in step (\ref{h_ref}) the $\zeta$-index array would be used
to interpolate in rows 10, 9, 8, 7, 6, 4, 3, 1, 1, 2, 4, 5, 7, 8, 9, 10
to calculate along the line of sight.

Ultimately, a similar $\zeta$-index, this one also accounting for GL
points, would be used in step (\ref{final}).
