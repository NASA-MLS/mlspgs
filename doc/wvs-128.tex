...
\newcommand{\docname}{wvs-128r3}
\newcommand{\docdate}{17 December 2015}
...
\item Construct the QTM top down, starting with level 1.  If the facet has
      a point in the polygon, or the polygon has a point in the facet, or
      an edge of the polygon intersects an edge of the facet, and the
      level of refinement is not sufficient, refine that facet.
...
\item \label{step3} When a facet is constructed, add its vertices that are
      ...
      level, and it has any points in the polygon, add all its points to
      the set.  Points of the QTM that surround the polygon are in the set
      so that it is possible to interpolate to any point within the
      polygon or on its boundary.  Use a hash table to avoid duplicates. 
      The serial number of a vertex is then used to compute a column
      subscript in the Jacobian, and a subscript in the state vector.
...
\item When a value is needed at a point specified by its longitude and
      ...
      any vertices not within the polygon.
      ...
      values, i.e., ``constant extrapolation'' isn't uniquely defined.
      ...
      the point of interest.  Then, construct lines that are perpendicular
      to the two edges incident on that vertex.  If either of those lines
      intersects an incident edge between its vertices, use the
      intersection that is closer to the point of interest (there might be
      two such intersections if the polygon is not convex); otherwise, use
      the nearest vertex.  By step \ref{step3} of the construction, this
      point will be within a facet.  Interpolate within that facet to that
      point.
