...
\newcommand{\docname}{wvs-128r2}
\newcommand{\docdate}{15 December 2015}
...
opposite the edge in common with the central facet appended.  This is also
the basis number of the vertex of the larger facet that is a vertex of the
smaller facet.
...
\item When a facet is constructed, add its vertices that are within the
      ...
      vector.  If the facet is at the specified refinement level, and it
      has any points in the polygon, add all its points to the set. 
      Points of the QTM that surround the polygon are in the set so that
      it is possible to interpolate to any point within the polygon or on
      its boundary.  Use a hash table to avoid duplicates.  The serial
      number of a vertex is then used to compute a column subscript in the
      Jacobian, and a subscript in the state vector.
...
\item When a value is needed at a point specified by its longitude and
      ...
      QID to traverse the QTM tree to find the facet.  The cost of both of
      these steps is proportional to the level of refinement, not the size
      of the QTM.  When the facet is found, the vertex of the QTM tree
      includes the serial numbers of its vertices, if it has any vertices
      within the polygon, or at least one missing serial number if it has
      no vertices within the polygon.

      If the specified point is within a facet that has serial numbers for
      its vertices, use interpolation.  Otherwise use extrapolation.

      In one dimension (either along-track or vertically in the orbit
      plane), our practice is to use constant extrapolation outside the
      region.  In two dimensions, a point outside the polygon might be
      close to vertices of the polygon that have different values, i.e.,
      ``constant extrapolation'' isn't uniquely defined. 

      Attempting to assign values to all vertices of facets outside the
      polygon, so that it is possible to interpolate to points within
      them, doesn't work because an edge of a larger facet might include
      vertices of several smaller facets.

      To extrapolate outside the polygon, construct lines through the
      desired point that are perpendicular to each edge of the polygon,
      until edges are found such that the constructed line intersects an
      edge between its end points, and the line does not intersect another
      edge at a point nearer to the specified point.  There might be more
      than one if the polygon is not convex.  Interpolate to those
      intersections and use the sum of those values, each one weighted by
      its distance from the desired point, divided by the sum of their
      distances.  If there is no such edge, use the value at the polygon
      vertex nearest to the specified point.
