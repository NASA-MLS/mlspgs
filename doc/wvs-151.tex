...
They can be thought of as a matrix in which each row corresponds to a
position on the path of integration, and each column corresponds to a
state-vector basis element.
...
A} and {\bf B} $\Rightarrow$ only two nonzero columns in row {\bf X}.
...
Compute the matrix-vector product {\bf P} of a sparse matrix {\bf S} with the
vector {\bf V}:
...
$\eta^k_{lm}$ from $\zeta$ and $\phi$ ones $\eta^k_l$ and $\eta^k_m$:\\[3pt]

{\tt~~call eta_zp(k)\%eta_nd ( eta_z(k), eta_p(k) )}\\[3pt]

or compute $f \times \zeta \times \phi$ ones $\eta^k_{\nu l m}$ from
$\eta^k_\nu$ and $\eta^k_{lm}$:\\[3pt]
...
of the construct.  When execution of the body completes, control resurns
