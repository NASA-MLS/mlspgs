...
\newcommand{\docname}{\bf wvs-056r3}
\newcommand{\docdate}{1 August 2007}
...
\mathcal{N}(h) = \,&1+n \text{ where}\\
n =\,& n_{i-1} e^{\epsilon(h-H_{i-1})}
   ...
   = n_i^q n_{i-1}^{1-q}\\
q =\,& \frac{h-H_{i-1}}{H_i-H_{i-1}} \text{, and}\\
...
$\frac{\text{d}\mathcal{N}}{\text{d}h} = \epsilon E$ we have
\begin{equation}\label{five}
f(x) = \frac1{1 + n(1 + h \epsilon)} \text{ and }
I = \int_{x_{i-1}}^{x_i} \frac1{1 + n(1 + h \epsilon)}\, \text{d}x\,.
\end{equation}
Remember that $h$ is a function of $x$.

When $n_{i-1} \approx n_i,\, \epsilon \approx 0$, and therefore $N \approx
...
Evaluating the integral in Equation (\ref{five}) using Gauss-Legendre quadrature
gives $I = \int_{x_{i-1}}^{x_i}\frac1{1+n+h\epsilon n}\,\text{d}x
\approx (x_i-x_{i-1}) \sum_{j=1}^3 f_j\, \omega_j$,
where $f_j = f(g_j)$ and $\omega_j$ is a Gauss-Legendre weight.
...
\frac{\text{d}f}{\text{d}x} = \,&
  -f^2 \left( \frac{\text{d}}{\text{d}h} ( 1 + n(1 + h \epsilon)) \right)
  \frac{\text{d}h}{\text{d}x}
= -f^2 \left( \frac{\text{d}n}{\text{d}h} ( 1 + h \epsilon ) + \epsilon n
  \right)
  \frac{\text{d}h}{\text{d}x}\\
= \,& -f^2 ( \epsilon n ( 2 + h \epsilon ) ) \frac{\text{d}h}{\text{d}x}
= -f^2 x \frac{\epsilon n ( 2 + h \epsilon )}
              {h ((1+n)^2 + h \epsilon n)}
= -f^2 x \frac{\epsilon n ( 2 + h \epsilon )}
              {h (1 + n^2 + n ( 2 + h \epsilon ))}
\end{split}\end{equation}
...
\left. \frac{\text{d}f}{\text{d}x}\right|_{x=x_i} =
 -f(x_i)^2 x_i \frac{\epsilon n_i (2 + H_i \epsilon)}
                      {H_i(1+n_i^2+n_i(2+H_i \epsilon))} =
 -f(x_i)^2 x_i \frac{\epsilon n_i (2 + H_i \epsilon)}
                      {H_i(1+n_i(n_i+2+H_i \epsilon))}
\end{equation}
...
water, and write out the linear interpolations for $T$ and $\zeta$ as a
function of $h$ explicitly using $q$ from Equation (\ref{three}), \emph{viz}.
\begin{equation}\label{nine}
n(h) = \frac{0.0000776}{T_{i-1}+q(T_i-T_{i-1})}
    \frac{10^{-\zeta_{i-1}}}{10^{\,q(\zeta_i-\zeta_{i-1})}}.
\end{equation}
Substitute $q=0$ in Equation (\ref{nine}) giving
\begin{equation}
...
\frac{n(h)}{n_{i-1}}=
  \frac{T_{i-1}}
       {T_{i-1}+q\,(T_i-T_{i-1})}10^{\,q(\zeta_{i-1}-\zeta_i)}
= \frac{T_{i-1}}
       {T_{i-1}+q\,(T_i-T_{i-1})}e^{\,q(\zeta_{i-1}-\zeta_i) \ln 10}.
\end{equation}
Substituting $q=1$ in Equation (\ref{twelve}), taking logarithms, and
re-arranging, we have
\begin{equation}
(\zeta_i-\zeta_{i-1}) \ln 10 =
\ln\left(\frac{T_i}{T_{i-1}}\frac{n_i}{n_{i-1}}\right) = \rho.
\end{equation}
Substituting this result in Equation (\ref{twelve}) and taking the derivative
with respect to $h$, we have
\begin{equation}\begin{split}\label{fourteen}
n(h) =\,& n_{i-1}\, \frac{T_{i-1}}
                   {T_{i-1}+q\,(T_i-T_{i-1})}
     e^{q\rho}\\
\,&\\
...
\frac{n(h)}{H_i-H_{i-1}}
\left(\rho - \frac{T_i-T_{i-1}}{T_{i-1}+q(T_i-T_{i-1})} \right).
\end{split}\end{equation}
Assuming we have $\mathcal{N}_i$, $T_i$ and $H_i$, we do not need
$\zeta_i$ to evaluate $n(h)$ or $\frac{\text{d}\mathcal{N}(h)}{\text{d}h}$.

Equation (\ref{fourteen}) reduces to the results of Section \ref{first} when
$T_i = T_{i-1}$.
