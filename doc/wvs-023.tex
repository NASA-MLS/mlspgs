...
\newcommand{\docname}{\bf wvs-023r3}
\newcommand{\docdate}{8 September 2005}
...
If {\tt Spect\_der=true} or {\tt /Spect\_der} is specified in the forward
model configuration, and a {\tt Jacob\-ian} matrix is specified in a
{\tt  Retrieve} or {\tt SIDS} specification that uses that configuration,
derivatives of radiance with respect to these quantities are computed,
which allows to solve for them in the retrieval case if they are in the
{\tt State} vector.  Specifying a Jacobian matrix alone is not sufficient
to cause derivatives to be computed.
