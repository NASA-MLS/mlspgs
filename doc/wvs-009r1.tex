...
\newcommand{\ps@twolines}{%
  \renewcommand{\@oddhead}{%
    8 November 2000\hfill Page \thepage\ of \pageref{lastpage}\hfill{\bf
    wvs-009r1}}%
...
this again squares the already-squared condition number.)  It turns out
not to be necessary.  Start with $\mathbf{U \delta x} = \mathbf{U^{-T} c}
= \mathbf{y}$ and add new rows $\delta \lambda \mathbf{I} \mathbf{\delta
x} = 0$ to form the least-squares problem
...
Since the solution includes both volume mixing ratios and temperatures,
it is expected that there will be variation of approximately 14 orders of
...
least-squares problem as $\mathbf{A \Sigma \Sigma^{-1} \delta x} \simeq
...
\Sigma^{-1} \delta x}$, and leaving aside the Levenberg-Marquardt
stabilization for the moment, we have, in normal equations form,
$\mathbf{\Sigma^T ( \hat A^T \hat A + S^{-1} ) \Sigma y = \Sigma^T ( \hat
A^T \hat b + \hat a)}$.
...
Another possibility is to use $\mathbf{\Sigma = W_a^{-1}}$.  Working
...
A third possibility is to assume that $\mathbf{a}$ is a good
approximation for $\mathbf{x}$, and therefore to scale with a diagonal
$\mathbf{\Sigma}$ the diagonal elements of which are corresponding
elements of $\mathbf{a - x^{(r)}}$.  If we have faith in the a priori
estimate, this one produces the best scaling of the solution, with the
least work.
...
\Sigma}$ (where $\mathbf{A}$ is the matrix of this section, not the
earlier sections).
