...
% -----------------------------------------------------------------------------
%
\slidetitle[Least squares]{Nomenclature for next slides}
\begin{itemize}
\item $i$: MAF number
...
% ----------------------------------------------------------------------------
%
\slidetitle{Introduction to Newtonian iteration}
...
\end{equation}
The Newtonian iteration for finding a zero of $f$ is gotten by setting $f(x)$
...
% ----------------------------------------------------------------------------
%
\slidetitle{Least-squares view of problem (1)}
...
% ----------------------------------------------------------------------------
%
\slidetitle{Least-squares view of problem (2)}
In broad outline, the least-squares problem to be solved in each Newtonian
iteration consists of four parts:
\begin{equation}
 \begin{split}
   \b{J} \delta \b{x}         &\simeq - \b{f} \\
   \b{F} \b{x_{n+1}}          &\simeq \b{F a} \\
   \b{R} \b{x_{n+1}}          &\simeq 0 \\
...
% ----------------------------------------------------------------------------
%
\slidetitle{Least-squares view of problem (3)}
The Newtonian iteration consists of evaluating $\b{K}$ and $\b{f}$ at
$\b{x}_n$, solving $\b{K}\delta\b{x}=-\b{f}$ to get $\b{x}_{n+1}$, and
repeating until $|\b{f}|$ or $|\delta\b{x}|$ is ``small enough''.  In
detail:
\begin{equation*}
...
% ----------------------------------------------------------------------------
%
\slidetitle[Solving]{Solving the least-squares problem}
...
of needing to form the normal equations.
%
Alternatives are
\begin{itemize}
...
% ----------------------------------------------------------------------------
%
\slidetitle{Accumulating normal equations little-by-little}
...
\item Accumulating normal equations little-by-little amounts to nothing
  more than running the summation from $1$ to $m_1$ for all $i$ and $j$,
  then from $m_1+1$ to $m_2$ for all $i$ and $j$, etc., i.e.\ changing the
  order of addition.
...
% ----------------------------------------------------------------------------
%
\slidetitle[Algebra]{Organization of the linear algebra software}
\begin{itemize}
...
% ----------------------------------------------------------------------------
%
\slidetitle{Organization of the linear algebra software (cont.)}
