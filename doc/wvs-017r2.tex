...
\newcommand{\docnum}{wvs-017r2}
\newcommand{\docdate}{18 June 2004}
...
respectively. The set \{weak\} is defined such that for each $k \in$ \{weak\},
$\delta_{jkn}$ changes slowly as a function of $n$, i.e., it represents a weak
...
Since $\delta_{jkn}$ changes slowly as a function of $n$ for $k \in$
\{weak\}, we can replace $\delta_{jkn}$ for $k \in$ \{weak\} by some kind of
average w.r.t. $n$, \emph{viz}. $\overline{\delta_{jk}^w}$.
...
$\overline{\delta_j^w} = \sum_{k\in\text{weak}} \overline{\delta_{jk}^w}$ and
...
The $\overline{\delta_j^w}$ quantities can be interpolated from tables that can
be calculated off line.  The remaining quantities are calculated on line.
