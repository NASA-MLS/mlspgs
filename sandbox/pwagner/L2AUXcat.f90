...
! LF95.Linux/test [options] [input files] -o [output file]
  ...
  if ( n_filenames == 0 ) then
    if ( verbose ) print *, 'Sorry no input files supplied'
  else
    call time_now ( t1 )
    if ( verbose .and. .not. list) print *, 'Copy l2aux data to: ', trim(outputFile)
    do i=1, n_filenames
      call time_now ( tFile )
      if ( list ) then
        ...
        if ( verbose ) then
          print *, 'Copying from: ', trim(filenames(i))
        endif
        call cpL2AUXData(trim(filenames(i)), &
