module PFA_FWD_MDL_M
  ...
  use ELLIPSE, only: EARTHX, HT, HT2, NPHI_TAN, NPHI_S, PHI_S, PHI_TAN, &
                     ROC, RR
  use EOS_MDB, only: EOS_MDB_HDR, EOS_MDB_REC, MAX_NO_LINES, MNF => max_freq
...
!
   ...
   &                        DHDZ_PATH(Npath,*)
    ...
    Real(r4), intent(in) :: Z_GRID(*), T_GRID(*), A_GRID(*)
    Real(r4), intent(in) :: CONV_HTS(*)
    Real(r4), intent(in) :: CEN_ANGLE
    ...
    Real(r4), intent(out) :: SCALE_FACTOR(*)
    Real(r4), intent(inout) :: I_STAR_ALL(*)
    Real(r4), intent(out) :: K_STAR_ALL(msvd,mnp,*)
    integer(i4), intent(out) :: IER
!
    character(len=24) :: AX
    real(r4) :: A, A_63, A2B2, AZIM_ANGLE, B, B2
!   real(r4) :: beta_t_power(N2lvl,maxaitkenpts,maxpfach,Nsps,Nptg)
    real(r4) :: beta_t_power(N2lvl,maxaitkenpts,2,2,Nptg)
    character(len=1) :: CA
    real(r4) :: C_D_ELEV, C_E
    integer(i4) :: COMP_NDX, CON_TYPE
    real(r4) :: CSE
    real(r4) :: DEL_OPCTY(N2lvl), DELTA(N2lvl,mxco,mnp,Nsps), DRADZ(Nptg)
    real(r4) :: E_Z, ELEV_OFFST
    real(r4) :: FFT_ANGLES(maxfft), FFT_PRESS(maxfft)
...
!
   ...
   &       k_star_spect_dnu(Nptg,mxco,mnp,2)
    ...
    integer(i4) :: M
    integer(i4) :: NF, NTR
!   real(r4) :: pfa_beta_coeff(N2lvl,maxaitkenpts,maxpfach,Nsps,Nptg)
    real(r4) :: pfa_beta_coeff(N2lvl,maxaitkenpts,2,2,Nptg)
!   real(r4) :: pfa_dbeta_dn(N2lvl,maxaitkenpts,maxpfach,Nsps,Nptg)
    real(r4) :: pfa_dbeta_dn(N2lvl,maxaitkenpts,2,2,Nptg)
!   real(r4) :: pfa_dbeta_dnu(N2lvl,maxaitkenpts,maxpfach,Nsps,Nptg)
    real(r4) :: pfa_dbeta_dnu(N2lvl,maxaitkenpts,2,2,Nptg)
!   real(r4) :: pfa_dbeta_dw(N2lvl,maxaitkenpts,maxpfach,Nsps,Nptg)
    real(r4) :: pfa_dbeta_dw(N2lvl,maxaitkenpts,2,2,Nptg)
    real(r4) :: PFA_RAD(Nptg)
    integer(i4) :: PTG_I
    real(r8) :: Q
    real(r4) :: R, RAD(maxaitkenpts)
    real(r4) :: Rad_dn(maxaitkenpts,mxco,mnp,Nsps)
    real(r4) :: Rad_dnu(maxaitkenpts,mxco,mnp,Nsps)
    real(r4) :: Rad_dw(maxaitkenpts,mxco,mnp,Nsps)
    real(r8) :: ROOT
    real(r8) :: RS ! For ellipse calculation
    real(r4) :: S_D_ELEV, S_E
    integer(i4) :: S_NP, S_NZ, SA, SHFT, SPECTAG, SV_C, SV_ELMNT
    real(r4) :: T_SCRIPT(N2lvl), TAU(N2lvl), TMPRAD(Nptg)
    real(r4) :: VRAD(maxfft)
!
! -----     Statement functions     ------------------------------------
!
...
!
    Ier = 0
!
! Clear (zero out) all the beta_coeff array slots:
!
    do jz = 1, N2lvl
      do jtan = 1, maxaitkenpts
        do j = 1, 2                 ! Should be maxpfach
          do j4 = 1, 2              ! Should be Nsps
            do k = 1, Nptg
              beta_t_power(jz,jtan,j,j4,k) = 0.0
              pfa_dbeta_dw(jz,jtan,j,j4,k) = 0.0
              pfa_dbeta_dn(jz,jtan,j,j4,k) = 0.0
              pfa_dbeta_dnu(jz,jtan,j,j4,k) = 0.0
              pfa_beta_coeff(jz,jtan,j,j4,k) = 0.0
            end do
          end do
        end do
      end do
...
!
! Clear (zero out) all the k_star_spect arrays:
!
    do ptg_i = 1, Nptg
      pfa_rad(ptg_i) = 0.0
      do jz = 1, 2           ! Should be Nsps
        do k = 1, mnp
          do j = 1, mxco
            k_star_spect_dw(ptg_i,j,k,jz) = 0.0
            k_star_spect_dn(ptg_i,j,k,jz) = 0.0
            k_star_spect_dnu(ptg_i,j,k,jz) = 0.0
         end do
        end do
      end do
    end do
...
! *** DEBUG
!
    if (j4 > -2) goto 19    ! Bypass dumping of value
!
    ntr = 41
!   jz = (j4-1) / 2
    jz = 1
...
!
    e_z = -1.0
    write(*,901) e_z
901 format(4x,'Variable Temperature, Fixed Zeta =',f7.4,/,              &
   &       4x,'Temp',7x,'dbeta_dw',6x,'dbeta_dn',6x,                    &
   &       'dbeta_dnu0',/,65('='))
!
    a2b2 = 100.0/(ntr-1)
    do jf = 1, ntr
      a_63 = 200.0+a2b2*(jf-1)
      Call Create_beta_coeff(band,1,e_z,a_63,q,pfa_spectrum,            &
   &                         mdb_hdr,mdb_rec,a,b,b2,c_e,s_e)
      write(*,911) a_63,b2,c_e,s_e
    end do
!
911 format(2x,f8.4,3(2x,1pg13.6))
!
    a_63 = 250.0
    write(*,902) a_63
902 format(4x,'Variable Zeta, Fixed Temp =',f8.3,/,                    &
    &       4x,'Zeta',7x,'dbeta_dw',6x,'dbeta_dn',6x,                   &
    &       'dbeta_dnu0',/,65('='))
!
     a2b2 = 7.0/(ntr-1)
     do jf = 1, ntr
...
!  this tanget:
!
        Call do_t_script(N_lvls,Ng,Frq,sp_tmp,t_path(1,k),IndxR,        &
   &                     IndxL,path_brkpt(1,k),t_script)
!
   ...
   &         f_basis,Sps_Coef_Loop,ref_corr(1,k),                       &
   &         mnp,no_freqs_f,no_phi_f,phi_basis_f,IndxR,                 &
   &         IndxL,path_brkpt(1,k),beta_t_power(1,1,1,1,k),delta,Ier)
        if (Ier /= 0) Return
!
...
!
   ...
   &         N_lvls,N2lvl,mxco,mnp,no_phi_f,delta,del_opcty,            &
   &         IndxR,IndxL)
!
        Call Scrt_dn(t_script,N_lvls,cse,del_opcty,tau,Rad(jf),         &
   &                 IndxR,IndxL,ilo,ihi)
!
        del_opcty(1:N2lvl) = 0.0
!
...
!
   ...
   &                 ilo,ihi,s_nz,s_np,spectroscopic(sa),              &
   &                 Rad_dw(1,1,1,nf),Ier)
                if (Ier /= 0) Return
!
              else if (CA == 'N') then
...
!
      Pfa_rad(ptg_i) = Aitken_int(f_grid,f_grid_filter,filter_func,   &
   &                              Rad,j4,No_filt_pts,Ier)
      if (Ier /= 0) Return
!
...
!
   ...
   &                    no_conv_hts,band,fft_pts,InDir,Aaap,Ier)
      if (Ier /= 0) Return
!
    end if
!
...
!
    ...
    Call Cspline_der(fft_press,ptg_press%lin_val,VRad,TmpRad,         &
   &                  dRadz,ktr,j)
!
...
!
                do ptg_i = 1, kconv
                  if (CA == 'W') then
                    r = k_star_spect_dw(ptg_i+shft,iz,ip,nf)
                  else if (CA == 'N') then
                    r = k_star_spect_dn(ptg_i+shft,iz,ip,nf)
                  else if (CA == 'V') then
                    r = k_star_spect_dnu(ptg_i+shft,iz,ip,nf)
                  end if
                  VRad(ptg_i) = r
                end do
!
...
!
                  Call fov_convolve(fft_angles,VRad,cen_angle,         &
     &                 con_type,kconv,band,fft_pts,InDir,Aaap,Ier)
                  if (Ier /= 0) Return
!
...
!
                Call Lintrp(fft_press,ptg_press%lin_val,VRad,          &
     &                        k_star_all(sv_elmnt,ip,1:j),ktr,j)
!
              end do
!
            end do
...
!
          ...
          s_nz = spectroscopic(sa)%no_zeta_values
          r = -1.0
          Call Hunt(r,spectroscopic(sa)%zeta_basis,s_nz,iz,ihi)
...
918   Format(a,i2.2,'_phi_',i1,a1,i2,'n')
!
