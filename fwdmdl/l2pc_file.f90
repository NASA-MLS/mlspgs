Program L2PC_FILE
  ...
  use L2PC_PFA_STRUCTURES, only: ATMOS_COMP, GEOM_PARAM, GEOPHYS_PARAM, &
                                 LIMB_PRESS, MAXAITKENPTS, MAXFILTPTS, MAXGEOM, MAXGEOPHYS, MAXPFACH, &
...
!
  ...
  real(r4) :: ACC(maxpfach)                         ! PFA
  character(len=40) :: ACON, AKEYS(17)
  integer(i4) :: ATMOS_INDEX(Nsps)
  type (atmos_comp) :: ATMOSPHERIC(Nsps)
  real(r4) :: AZIM_REF, AZIM_183, AZIM_205          ! PFA
  integer(i4) :: BAND, BAND_1, BAND_2
  character(len=80) :: BUFF
  real(r8) :: C_FREQ(Nch)
  real(r4) :: C_PITCH, C_ROLL, C_YAW                ! PFA
  real(r4) :: CEN_ANGLE
  integer(i4) :: CH1, CH2
  real(r4) :: CONV_HTS(Nptg), CONV_HTS_RAW(Nptg), CONV_HTS_SAVE(Nptg)
  integer(i4) :: CONV_INDX(Nptg)
  real(r4) :: CONV_PRESS(Nptg), CONV_TEMP(Nptg)
  real(r4) :: CS(Nlvl,no_t_phi,mnf,Nsps)
  character(len=80) :: DFILENAME
  integer(i4) :: DCH1, DCH2
  real(r4) :: DH_DT_GRID(Nlvl,mxco)
! Real(r4) :: DH_DT_PATH(Npath,mnp,mxco,Nptg)
  ...
  real(r4) :: DX_DT(Nlvl,mxco)
  real(r4) :: D2X_DXDT(Nlvl,mxco)
  real(r4) :: EARTH_RADIUS, EARTH_REF
  real(r4) :: ELEV_183, ELEV_205                    ! PFA
  real(r4) :: F_BASIS(mxco,Nsps)
  ...
  real(r4) :: G_BASIS(mxco,Nsps)
  real(r4) :: GEOCSRAD                              ! PFA
  ...
  real(r4) :: H, H_GNLV(max_no_zeta), H_GRID(Nlvl), H_OBS
  real(r4) :: H_PATH(Npath,Nptg)
  ...
  real(r4) :: MDB_PRES(Nlvl)
  type (eos_mdb_rec) :: MDB_REC(max_no_lines,02)     ! 2 should be: maxsps
  real(r4) ::MDB_TEMP(Nlvl)
  real(r4) :: MR_F(mxco,mnp,Nsps), MR_G(mxco,mnp,Nsps)
  ...
  real(r4) :: PHI_BASIS_F(mnp,Nsps), PHI_PATH(Npath,Nptg)
  type (pfa_slab) :: PFA_SPECTRUM(6,Nsps)
  character :: PRIMAG
  real(r4) :: PTG_ANGLE(Nlvl), PTG_HTS(max_no_pointings)
  integer(i4) :: REC_NOS(max_no_key_addr)
  real(r4) :: REF_CORR(Nlvl,Nptg)
  real(r8) :: ROC
  character(len=80) :: RUNF
  real(r4) :: S_PITCH, S_ROLL                       ! PFA
  real(r4) :: S_TEMP
  real(r4) :: S_YAW                                 ! PFA
  integer(i4) :: SI, SPECT_ATMOS(Nsps), SPECT_INDEX(Nsps)
  type (spectro_param) :: SPECTROSCOPIC(3*Nsps)
  real(r4) :: SPSFUNC(Nlvl,Nsps)
  integer(i4) :: SPS_TBL(Nsps,max_no_bands), SV_I
  real(r4) :: T_GNLV(max_no_zeta), T_GRID(Nlvl)
  integer(i4) :: T_INDEX
  real(r4) :: T_PATH(Npath,Nptg), T_PHI_BASIS(mnp)
  real(r4) :: T_TAN(Nlvl), THRESHOLD(maxpfach)      ! PFA
  integer(i4) :: TIME_I
  character(len=8) :: TIME_STAMP, TS, TT
  character(len=80) :: UIF_NAME
  real(r4) :: V_GRID(Nlvl), VEL_Z
  real(r4) :: Z, Z_GNLV(max_no_zeta), Z_GRID(Nlvl), Z_PATH(Npath,Nptg), ZREF
!
! For timing:
!
  intrinsic :: CPU_TIME
...
!
  do i = 1, no_geophys
    ...
    do while(j < k .and. band < 0)
      ...
      Call s_hunt(z,z_grid,N_lvls,m,idt)
...
!
  ...
  if(idt == 0) then                ! run file does not exist
    ...
    Acon = 'Re-Started at:'
    open (86,file=runf(1:lrun),access='APPEND',iostat=ios)
  end if
!
  Ts(1:)=' '
  Buff(1:) = ' '
  Lines(1) = Buff
  j = len_trim(Acon)
  Buff = '  "Crash Proof" EOS Version L2PC RUN. '//Acon(1:j)
  i = len_trim(Buff)
!
  Acon(1:)=' '
  Call Date(Acon)
...
!
 ...
 &       dh_dt_path,Npath,path_brkpt,no_phi_t,t_phi_basis,RoC,         &
...
!
    Call refraction_correction(N_lvls,no_conv_hts,h_grid,t_grid,       &
 &                  n_grid,conv_hts,earth_radius,ref_corr)
!
...
!
888 close (86,iostat=ios)
  close (88,iostat=ios)
  close (l2pc_lu,iostat=ios)
...
!
  Ier = iabs(Ier)
  if(Ier == 0 .or. Ier > 1000) then
    Call Get_Rid('l2pc_run.dat')
    Call Get_Rid('l2pc_dump01.dat')
  end if
!
