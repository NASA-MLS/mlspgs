...
!   real(rp), parameter :: gx = 1.0_rp / sqrt2
!   real(rp), parameter :: gw = 0.5_rp / sqrtPi

    ! For 3-pt Gauss-Hermite
...
!{    {\tt CLorentz} is one term of the asymptotic expansion
...
!     accurate enough for derivative computations.
      ...
      if ( present(du) .or. present(dv) ) then
        ...
        !       i \left[a r  -\frac12( r q + s p ) \right]$.
...
! Real Voigt region IV 2pt Gauss-Hermite integration
...
!   Internals
    ...
    ! Two-point Gauss-Hermite formula
    real(rp), parameter :: gx = 1.0_rp / sqrt2
    real(rp), parameter :: gw = 0.5_rp / sqrtPi
    real(rp) :: y2
...
! Voigt region  2pt Gauss-Hermite integration
...
! Real Voigt region IV 6pt Gauss-Hermite integration
...
! Voigt region IV 6pt Gauss-Hermite integration
...
! $Log$
! Revision 2.15  2011/08/19 21:01:52  vsnyder
! Correct comments, especially TeXnicalities
!
