...
! -------------------------------------------------  GET_BETA  -----
      SUBROUTINE GET_BETA(QLG,V0,GSE,IST,WTH,NTH,DELTA,N1,GAMMA,N2, &
                 &        NMOL,NCNT,T,PB,F,RH,VMR_O3,VMR_N2O,ABSC )
...
!      use WaterVapor, only: RHtoEV
      ...
      REAL(r8), parameter :: VMR_O2 = 0.209476_r8 ! O2 VOLUME MIXING RATIO
      REAL(r8) :: B                           ! BETA (1/m/ppv)
      REAL(r8) :: RH                          ! Relative Humidity if values > 1
                                              ! or H2O vmr if values < 1
      REAL(r8) :: VMR_O3                      ! O3 VOLUME MIXING RATIO
...
!----------------------------------------------------------------
     ...
     &          (1._r8 - EXP(-V01(i)/(20836.7_r8*300.0_r8)))/T 
         ...
         B=MAX(0._r8,B)                 ! AVOID NEGATIVE LINE SHAPE     
            ...
            case (5)
              ABSC = ABSC + B*VMR_O3                   ! O3
            case (6)
              ABSC = ABSC + B*VMR_N2O                  ! N2O
            end select
...
!
! Revision 2.1  2003/11/17 17:46:37  jonathan
! moved from cloudfwdm to fwdml
!
