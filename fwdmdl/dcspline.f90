...
!
    real(r8), intent(in) :: XIN(abs(nin)), XOUT(nout), YIN(abs(nin))
    real(r8), intent(out) :: YOUT(:)
!
    integer :: I, JIN, KHI, KLO, KPL
!
    real(r8) :: COEFF(4, abs(nin)), X_IN(abs(nin))
    real(r8) :: X, XL, XH, YL, YH, YLIN, YSPL, DYDX, DP, H
!
...
!-------------------------------------------------- pcspl ------------
!
  subroutine PCSPL ( TAU, C, N, IBCBEG, IBCEND )
!
...
!
    ...
    real(r8), intent(inout) :: C(4,*)
!
    integer :: L, M, J, I
    real(r8) :: DTAU, R, G, DIVDF1, DIVDF3
!
