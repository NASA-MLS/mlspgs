...
!   integer(i4) :: ICH                   ! FOR DEBUGGING
    ...
    real(r4) :: TERM(Nlvl), TX(1), TY(1)
!
! -----     Generic interfaces     -------------------------------------
!
! These compensate for the use of ACOSD, COSD and SIND,
! which are intrinsic in some compilers, but are not part of any standard.
!
  interface ACOSD; module procedure DACOSD, SACOSD; end interface
  interface COSD; module procedure DCOSD, SCOSD; end interface
  interface SIND; module procedure DSIND, SSIND; end interface
!
! -----     Begin the code     -----------------------------------------
!
...
!
    ...
    Call Cspline_der(Press,ptg_press%lin_val,Rad,i_star_all,sc3,      &
   &                  kconv,j)
!
...
!
            Call Lintrp(conv_press,ptg_press%lin_val,Rad,sc1,        &
   &                      kconv,j)
...
!
            Call Cspline(conv_press,ptg_press%lin_val,      &
   &                     Rad,k_star_all(sv_elmnt,nf,1:j),   &
   &                     kconv,j)
!
          end do
!
...
! Get X^63
!
    Call Lintrp(conv_press,ptg_press%lin_val,ptg_angles,Ang,         &
   &              kconv,j)
!
! Get dI/dX at 63
!
...
! which are intrinsic in some compilers, but are not part of any standard.
!
  pure real(r8) function DACOSD ( X )
    real(r8), intent(in) :: X
    dacosd  = rad2deg * acos(x)
  end function DACOSD
  pure real(r8) function DCOSD ( X )
    real(r8), intent(in) :: X
    dcosd = cos(x*deg2rad)
  end function DCOSD
  pure real(r8) function DSIND ( X )
    real(r8), intent(in) :: X
    dsind = sin(x*deg2rad)
  end function DSIND
  pure real(r4) function SACOSD ( X )
    real(r4), intent(in) :: X
    sacosd  = rad2deg * acos(x)
  end function SACOSD
  pure real(r8) function SCOSD ( X )
    real(r4), intent(in) :: X
    scosd = cos(x*deg2rad)
  end function SCOSD
  pure real(r8) function SSIND ( X )
    real(r4), intent(in) :: X
    ssind = sin(x*deg2rad)
  end function SSIND
