...
!
 ...
 &           Ngp1, pfa_beta_coeff, beta_t_power, pfa_beta_Ntrp, t_power )
    ...
    real(r4), intent(in) :: PFA_BETA_COEFF(*)
    real(r4), intent(in) :: BETA_T_POWER(*)
    real(r4), intent(out) :: PFA_BETA_NTRP
...
!
      ...
      if (w > 1.0e-7_r8) then           ! Power rule for pressure
        r = d / (z_path(jh) - z_path(skng))
        Pfa_beta_Ntrp = Pfa_beta_Ntrp * q**r
      end if
!
    end if
!
