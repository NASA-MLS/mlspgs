C SUBROUTINES FOR CALCULATIONS OF SURFACE EMISSIVITY
...
C  S --- Salinity in per thousand if ocean,  
C        Emissivity value, if land (negative means default,0.9)
C        Ice type, if sea ice, 0=new,1=2nd year,2=multiyear
C        Snow type, if snow, 0=wet,1=dry,2=refrozen
C  W --- Wind Speed in m/s, if ocean
C        Not used, else
	...
	real RV(NP)	! reflectivity for vertical polarization
       ...
       if (IS.eq.0) then
c         call ASSEA1(F,TS,S,W,X,N,NP,RH,RV)
c... simple model
	 do i=1,np
	 rh(i) = 0.8
	 rv(i) = 0.8
...
C      S ---- SALINTY PER MILI
     ...
     1  -S*(1.849E-5-2.551E-7*D+2.551E-8*D*D)
...
CCF
     ...
     1      -1.187e-6*CA*CA*CA+7e-20*CA**10
...
c     good only for ssm/i channels
       REAL X(NP),RH(NP),RV(NP),C(8,4),DES(8),CG(4)
       DATA CF,W0,CA0,TS0/8.E-3,7.,53.,273./
       DATA C/-0.556,0.406,-0.670,0.479,
     1 -0.811,0.473,-0.723,0.358,
     2  0.357,-0.108,0.455,-0.175,0.551,-0.160,
     3  0.404,-0.0351,
     4 -0.0312,0.0128,-0.0446,0.0283,
     5 -0.0365,0.0312,-0.00735,0.0309,
     6  0.0106,0.00153,0.0232,-0.0131,
     7  0.0149,-0.0150,-0.0126,-0.0121/
c      DATA DES/0.004,0.00354,-0.00451,0.0,
c     1        -0.0124,-0.0125,-0.00189,0.05415/
       DATA DES/8*0.0/
...
C emissivity for non-ocean surface
C
       SUBROUTINE ASSEAN(F,TS,IS,S,X,N,NP,RH,RV)

C IS -- 1 = land, w=emissivity of it,0.9 if negative given
C       2 = ice, S---0: New, 1: Second Year, 2: Multiyear
C       3 = snow,S---0: Wet, 1: Dry, 2: Refrozen
     ...
     1     /(1+(F/CICE(3,k))**CICE(4,k))
        
       else if(IS.eq.3) then
         k=nint(S)+1
         e=(CSNOW(1,k)+CSNOW(2,k)*(F/CSNOW(3,k))**CSNOW(4,k))
     1     /(1+(F/CSNOW(3,k))**CSNOW(4,k))
       end if
