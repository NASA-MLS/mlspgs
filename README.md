...
Aura Microwave Limb Sounder (MLS) Level 1 and 2 product generation software.
...
For more information about Aura MLS, see:
...
Additional supporting information on Aura MLS can be found in related archived material, such as the former project Wiki, etc. which can be found ***WHERE***?
...
Level 2 is run in a "brain-dead parallel" manner, with the (admittedly dated) PVM (Parallel Virtual Machine) message passing system used for the (relatively lightweight) communication between the tasks.  Fundamentally, each CPU runs a single-threaded task to process a fraction (~15 degrees along the orbit track, about 4-5 minutes-worth of data).

The repository also contains (somewhat hidden in branches) a d iscontinued approach to generating "Level 3" data (geophysical products in on regular latitude/longitude grids).  The current approaches for generating Level 3 data (based on established software written in the IDL language) are archived elsewhere.
