...
! ------------------------------------------  CloudRetrieval  -----
  subroutine CloudRetrieval(Method, ConfigDatabase,configIndices,fwdModelExtra,&
      ...
      & FINDBLOCK, GetDiagonal, Matrix_T, &
      & Matrix_SPD_T, &
      & UpdateDiagonal
      ...
      use VectorsModule, only: CloneVector, &
       & DestroyVectorInfo, GetVectorQuantityByType, &
       & M_Cloud, &
       & Vector_T, VectorValue_T

      ! IO variables
      type(forwardModelConfig_T), dimension(:), pointer :: ConfigDatabase
      integer, dimension(:) :: ConfigIndices    ! In ConfigDatabase
...
! ----- executable ----
  ...
  subroutine LOS2Grid( Qty, vQty, vQtyApr, Los, vLos, vLosApr, Ptan, Re, Pcut)
    ...
    integer :: closestInstances(los%template%noInstances) ! NoMAFs
...
! $Log$
! Revision 2.15  2014/09/05 00:39:12  vsnyder
! Remove unnecessary POINTER attribute from some arguments.  Cannonball
! polishing.  Remove declarations for unused variables.  Convert some
! pointer temps to automatic.
!
