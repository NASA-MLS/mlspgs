...
contains ! =====     Public Procedures     =============================
  ...
  ! ------------------------------------------------  InitGetL2CF  -----
  ! Initialize routines needed by GetL2CF.  This is separate, because it
  ! clears the toggles.  You may want to set them before calling GetL2CF.

  subroutine InitGetL2CF ( nChars, nSymbols, nHash, nDecls, nTree )
    ...
    call init_tables
    call init_units
  end subroutine InitGetL2CF

  ! ----------------------------------------------------  GetL2CF  -----
  ! Parse a Configuration file, producing a data structure described in
  ! the MLSCF module.  InitGetL2CF must be called first.

  subroutine GetL2CF ( L2CF_Data, Status, InUnit, Listing, &
    &                  Dump, DumpEarly, DumpTree, DumpTables )
    type(mlscf_t), intent(out) :: L2CF_DATA  ! The L2CF data
    ...
    if ( root > 0 ) then
      ...
      if ( error == 0 ) then
        ...
        if ( do_dump_tables ) call dump_table ( l2cf_data )
      end if
    else
      error = -1
      call output ( &
  ...
  end subroutine GetL2CF
end module GetL2CF_M

! $Log$
! Revision 2.1  2000/09/29 23:28:59  vsnyder
! Initial entry
!
