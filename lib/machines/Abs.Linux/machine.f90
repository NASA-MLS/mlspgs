...
contains
  ...
  ! where x is the method
    ...
    if ( present(code) ) print *, 'In Absoft MACHINE%CRASH_BURN, CODE =', code
    if ( present(msg) ) print *, 'In Absoft MACHINE%CRASH_BURN, MSG = ', trim(msg)
...
!
! Revision 1.5  2004/08/19 00:13:12  pwagner
