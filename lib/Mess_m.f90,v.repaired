...
module Mess_M
  ...
  ! Extended error message processor from Math 77.

  implicit NONE

  private

  public DMESS, IMESS, MESS, SMESS

  interface MESS
    module procedure DMESS, IMESS, SMESS
  end interface

! ************** Parameters Defining Actions (See IMESS) ***************

      integer, public ::                                                &
     &          MESUNI, MEHEAD, MEDDIG, MEMLIN, MEELIN, MEMUNI, MEEUNI, &
...
!---------------------------------------------------------------------------
  ...
  ! Private stuff in common in the Fortran 77 version

      integer LENBUF
      parameter (LENBUF = 250)
      logical, save :: XARG, GOTFMT, XARGOK
      integer, save :: ERRCNT, EUNIT, ICHAR0, IRC, IVAR(MEVBAS:MEVLAS), &
     ...
     &   NDIM, NFDAT, NIDAT, NMDAT, NROW, NTEXT, OUNIT, SUNIT, TABSPA
      ...
      character, save :: BUF*(LENBUF), DOLS*72, FMTC*7, FMTF*20,        &
     &  FMTG*15, FMTI*7, FMTIM(2)*7, FMTJ*7, FMTR*7, FMTT*15

      equivalence (IVAR(MEVBAS   ), SUNIT)
      equivalence (IVAR(MEVBAS+1 ), LHEAD)
      equivalence (IVAR(MEVBAS+2 ), KDFDEF)
      equivalence (IVAR(MEVBAS+3 ), LINMSG)
      equivalence (IVAR(MEVBAS+4 ), LINERR)
      equivalence (IVAR(MEVBAS+5 ), MUNIT)
      equivalence (IVAR(MEVBAS+6 ), EUNIT)
      equivalence (IVAR(MEVBAS+7 ), KSCRN)
      equivalence (IVAR(MEVBAS+8 ), KDIAG)
      equivalence (IVAR(MEVBAS+9 ), MAXERR)
      equivalence (IVAR(MEVBAS+10), LSTOP)
      equivalence (IVAR(MEVBAS+11), LPRINT)
      equivalence (IVAR(MEVBAS+12), KDF)
      equivalence (IVAR(MEVBAS+13), NTEXT)
      equivalence (IVAR(MEVBAS+14), NIDAT)
      equivalence (IVAR(MEVBAS+15), NFDAT)
      equivalence (IVAR(MEVBAS+16), NMDAT)
      equivalence (IVAR(MEVBAS+17), MDAT)
      equivalence (IVAR(MEVBAS+18), TABSPA)
      equivalence (IVAR(MEVBAS+19), ERRCNT)
      equivalence (IVAR(MEVBAS+20), ICHAR0)
      equivalence (IVAR(MEVBAS+21), IMAG)
      equivalence (IVAR(MEVBAS+22), INC)
      equivalence (IVAR(MEVBAS+23), IRC)

      equivalence (FMTIM(1), FMTR), (FMTIM(2), FMTC)

      integer NROCO(2)
      equivalence (NROCO(1), NROW), (NROCO(2), NTEXT)
      integer LINSTR, KOLWID
...
! --------------------------------------------------------  IMESS  -----
  subroutine IMESS(MACT, TEXT, IDAT)
...
!>> 1&97-12-12 MESS  Krogh  Prefixed 0P edit descriptor to F format.
!>> 1&96-07-11 MESS  Krogh  Transpose matrix output for C.
!>> 1&96-06-27 MESS  Krogh  fprintf(stdout, => printf( & memset now used
!>> 1&96-06-18 MESS  Krogh  "Saved" NTEXTR.
!>> 1&96-05-15 MESS  Krogh  Changes to use .C. and C%%.
!>> 1&96-03-30 MESS  Krogh  Added external statement.
!>> 1&96-01-24 MESS  Krogh  Fixed minor bug introduced with "$ " stuff.
!>> 1&96-01-23 MESS  Krogh  Minor changes for C conversion.
!>> 1&95-11-10 MESS  Krogh  Add code to change "$ " to " " in headings.
!>> 1&95-08-11 MESS  Krogh  Made code default to not using UMESS.
!>> 1&95-01-20 MESS  Krogh  Fixed unusual case in matrix output.
!>> 1&94-12-15 MESS  Krogh  Removed block data for Cray T3D.
!>> 1&94-11-11 MESS  Krogh  Declared all vars.
!>> 1&94-09-14 MESS  Krogh  Fixed to get 1 more "$" in C output.
!>> 1&94-09-08 MESS  Krogh  Added new matrix/vector capabilities.
!>> 1&94-08-22 MESS  Krogh  Fix for conversion to C for new converter.
!>> 1&94-07-05 MESS  Krogh  Fixed bug, KDI and FMTI could be inconsist.
!>> 1&94-05-20 MESS  Krogh  Changes to MESSFT so line 1 can go to file.
!>> 1&94-05-20 MESS  Krogh  Changes to setting output unit.
!>> 1&94-05-09 MESS  Krogh  Integer vectors had overflow & space probs.
!>> 1&93-05-19 MESS  Krogh  Changed TEXT to array of character strings.
!>> 1&93-04-14 MESS  Krogh  Fixes for conversion to C. (C%% comments.)
!>> 1&93-03-10 MESS  Krogh  Broke into smaller pieces.
!>> 1&92-12-02 MESS  Krogh  Added save statement to block data subpr.
!>> 1&92-07-13 MESS  Krogh  Add checks in heading set up.
!>> 1&92-07-12 MESS  Krogh  Fixed so $$ prints a single $ in TEXT.
!>> 1&92-07-12 MESS  Krogh  Set out of bound inputs to limit values.
!>> 1&92-07-12 MESS  Krogh  Fixed so output works to alternate files.
!>> 1&92-07-12 MESS  Krogh  Added integer declarations for parameters.
!>> 1&92-06-24 MESS  Krogh  More blanks allowed on break of long lines.
!>> 1&92-06-10 MESS  Krogh  Minor fix to vector output.
!>> 1&92-05-27 MESS  Krogh  Fixed bug on line width setting.
!>> 1&92-05-14 MESS  Krogh  Put common blocks in save statement.
!>> 1&92-05-11 MESS  Krogh  Added label to assigned go to & a comment.
!>> 1&92-04-08 MESS  Krogh  Unused labels 60, 220 and 320 removed.
!>> 1&92-03-20 MESS  Krogh  Changed status on open to SCRATCH.
!>> 1&92-03-10 MESS  Krogh  1 line below label 690 changed max to min.
!>> 1&92-02-05 MESS  Krogh  Fixed bugs in printing matrix labels.
!>> 1&92-01-29 MESS  Krogh  Added UMESS and multiple print option.
!>> 1&91-12-09 MESS  Krogh  Fine tuning of vector output.
!>> 1&91-10-10 MESS  Krogh  Insure no stop if stop level = 9.
!>> 1&91-06-26 MESS  Krogh  Initial Code.
...
!   =&0  Get "maximum" valur for entries in a sparse vector.
!   =&1  Output a sparse vector.
...
! ************************** Variable Declarations *********************
     ...
     &    ITEXTR, ITXTSV, J, JJ, K, K1, K2, KDILAB, KK, KNT, KP,         &
     &    KS, LASKNT, LBUF1, LBUF2, M,                                   &
     &    MBNDHI(MEVBAS:MEVLAS), MBNDLO(MEVBAS:MEVLAS), MTEXT(2),        &
     &    MTEXTC, MTEXTR, NLINE, NSKIP, NTEXTR, NTXTSV
      logical   GETW, FIRST
...
!                                          Common code continues here
 1080 NLINE = (LENLIN - LINSTR + 1) / LENOUT
     ...
     &   LENOUT - mod(K-LINSTR, LENOUT)
      ...
      if (KLINE >= min(LASKNT-KNT, NLINE/2)) then
...
! I, K, KD are used in determining the number of characters needed to
!          represent IMAG.
...
!  calls MESS to do the actions in MACT.  Actions in MACT can not do
!  anything other than actions that reference MACT.
!  This routine is intended for use by library subroutines getting text
!  in the form of a Fortran character string.
...
!%%      memcpy(cmessc.buf, &ftext[cmessi.ntext-1], k);
      ...
      ICHAR0 = ICHAR('0')
      if (MACT(J) .ne. MECONT) call mess(MACT(J:J), TEXT, IDAT)
      return
